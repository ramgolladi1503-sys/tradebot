...
- Can absent exit data fabricate a result? No. The replay row receives `INSUFFICIENT_TRACE`; zero complete outcomes blocks the stage with a verified diagnostic artifact.
...
Acceptance requires all focused tests, Python compilation, direct pipeline command smoke, and all repository workflows to pass. The stage must block on wrong Truth lineage, same/pre-signal execution eligibility, absent bid or ask, crossed markets, duplicate trace timestamps, stale entry quotes beyond the configured delay, absent exit quotes, invalid or incomplete cost configuration, zero complete outcomes, changed input files, and undeclared extra inputs.
