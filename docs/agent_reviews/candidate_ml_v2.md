...
scope: Offline analytics dataset, model, validation, abstention, drift, counterfactual reporting, tests, documentation, and focused CI only.
...
- Added a candidate-level temporal feature dataset builder over recorded intent events and outcome-replay records.
- Added strict future-field, timestamp, chronology, support, and feature-completeness gates.
...
This PR must remain draft and unmerged until final-head CI passes and a human reviews the feature contract. It does not claim ML edge, profitability, paper readiness, or live readiness.
