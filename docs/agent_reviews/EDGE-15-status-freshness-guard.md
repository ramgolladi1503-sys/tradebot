...
reason: fresh runtime status must not inherit old auth-required WebSocket failure after newer live auth health confirms validation success
...
- make runtime auth snapshot prefer newer live auth health validation success over older persisted auth-required state
...
- older auth-required WebSocket failure is superseded by newer live auth health validation success
