...
The next safe step is a shadow-only canonical ledger workflow that writes snapshot, prediction, actual-open, score, and gap-response events through `core.analytics.store` into the repository's authoritative SQLite analytics backend. It must not write or read append-mode JSONL evidence directly. Outcome evaluation should remain behind the existing analytics replay/store boundary, and live/broker/order paths remain out of scope until separately reviewed and prospectively validated.
