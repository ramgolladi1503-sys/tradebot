#!/usr/bin/env python3
"""Primary mechanical replay for NIFTY_45DTE_VRP_V1.

The evaluator is intentionally narrow:
- real historical quotes only,
- one position per expiry,
- 42-48 calendar DTE entry window,
- ~16-delta short call + short put,
- sell at bid, buy back at ask,
- first 50% credit capture OR first 15:00 IST snapshot at <=21 DTE.

It emits descriptive research evidence only. It cannot certify edge, authorize
paper/live trading, or override the repository's BUY_ONLY production policy.
"""
...
def _select_leg(
    ...
    *,
    ...
    leg = leg.sort_values(
        ["_delta_distance", "delta", "_spread", "strike"],
...
def _synchronized_close_path(
    ...
    *,
    ...
    ce = exp_df.loc[
        (exp_df["type"] == "CE")
        & (exp_df["strike"] == ce_strike)
        & (exp_df["timestamp"] > entry_timestamp),
        ["timestamp", "ask", "bid", "delta"],
    ].copy()
    pe = exp_df.loc[
        (exp_df["type"] == "PE")
        & (exp_df["strike"] == pe_strike)
        & (exp_df["timestamp"] > entry_timestamp),
        ["timestamp", "ask", "bid", "delta"],
    ].copy()
...
def _time_exit_row(
    ...
    *,
    ...
    eligible = path.loc[
        path["trade_date"].map(lambda d: 0 <= (expiry_date - d).days <= time_exit_dte)
    ].copy()
    if eligible.empty:
        return None
...
def evaluate_expiry(
    ...
    *,
    ...
    initial_credit = entry["ce_bid"] + entry["pe_bid"]
    if initial_credit <= 0:
        return None
    ...
    target_hits = path.loc[path["close_debit"] <= target_debit]
    target_row = target_hits.iloc[0] if not target_hits.empty else None
    ...
    chosen = None
    reason = None
    if target_row is not None and time_row is not None:
        if pd.Timestamp(target_row["timestamp"]) <= pd.Timestamp(time_row["timestamp"]):
            chosen = target_row
    ...
    return {
        ...
        **entry,
        "initial_credit_points": float(initial_credit),
...
def _loss_concentration(values: pd.Series, n: int) -> float | None:
    ...
    if total <= 0:
        return None
    return float(losses.head(n).sum() / total)
...
def summarize(ledger: pd.DataFrame, *, dataset_sha256: str, config: dict) -> dict:
    if ledger.empty:
        return {
            "schema_version": "nifty_45dte_vrp_primary_eval_v1",
            "candidate_id": "NIFTY_45DTE_VRP_V1",
            "status": "PRIMARY_EVAL_NO_TRADES",
    ...
    for year, grp in ledger.groupby(ledger["entry_timestamp"].dt.year):
        ...
        by_year[str(int(year))] = {
            "trades": int(len(grp)),
            "mean_net_pnl_points": float(gpnl.mean()),
            "median_net_pnl_points": float(gpnl.median()),
            "win_rate": float((gpnl > 0).mean()),
            "total_net_pnl_points": float(gpnl.sum()),
            "worst_trade_points": float(gpnl.min()),
        }

    exit_counts = {str(k): int(v) for k, v in ledger["exit_reason"].value_counts().items()}
    ...
    return {
        ...
        "metrics": {
            "total_net_pnl_points": float(pnl.sum()),
            "mean_net_pnl_points": float(pnl.mean()),
            "median_net_pnl_points": float(pnl.median()),
            "win_rate": float((pnl > 0).mean()),
            "p05_net_pnl_points": float(pnl.quantile(0.05)),
            "p95_net_pnl_points": float(pnl.quantile(0.95)),
            "worst_trade_points": float(pnl.min()),
            "best_trade_points": float(pnl.max()),
            "max_drawdown_points": _max_drawdown(pnl),
            "worst_1_loss_share": _loss_concentration(pnl, 1),
            "worst_3_loss_share": _loss_concentration(pnl, 3),
            "bootstrap_95pct_mean_ci_points": _bootstrap_mean_ci(pnl.to_numpy()),
            "profit_target_hit_rate": float((ledger["exit_reason"] == "PROFIT_TARGET").mean()),
        },
        "exit_reason_counts": exit_counts,
        "by_entry_year": by_year,
        "interpretation_guardrails": [
            "Positive descriptive P&L is not structural-edge certification.",
            "Undefined-risk short-vol capital/margin normalization is not supplied by this evaluator.",
            "Statutory charges beyond executable bid/ask spread are included only if passed explicitly as extra cost points.",
            "The direct strategy conflicts with TradeBot BUY_ONLY production policy.",
            "Synthetic/realish option data is rejected before evaluation."
        ],
        "governance": {
            "edge_certified": False,
            "paper_authorized": False,
...
def run(
    ...
    *,
    ...
    config = {
        "entry_dte_window": [min_dte, max_dte],
        "snapshot_ist": snapshot_ist,
        "snapshot_tolerance_minutes": snapshot_tolerance_minutes,
        "target_abs_delta": target_abs_delta,
        "delta_tolerance": delta_tolerance,
        "profit_target_fraction_initial_credit": profit_target_fraction,
        "time_exit_dte": time_exit_dte,
        "entry_execution": "SELL_AT_BID",
        "exit_execution": "BUY_TO_CLOSE_AT_ASK",
        "extra_round_trip_cost_points": extra_round_trip_cost_points,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--time-exit-dte", type=int, default=21)
    p.add_argument("--extra-round-trip-cost-points", type=float, default=0.0)
    return p.parse_args()
...
def main() -> int:
    ...
    try:
        summary = run(
            ...
            extra_round_trip_cost_points=args.extra_round_trip_cost_points,
        )
        ...
        (args.output_dir / "primary_eval_summary.json").write_text(
