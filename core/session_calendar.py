...
_CAS_EFFECTIVE_DATE = date(2026, 8, 3)


def _default_sessions() -> Dict[str, SessionSpec]:
    return {
        "NSE_EQ": SessionSpec(_IST, dt_time(9, 15), dt_time(15, 35), "NSE Equity CAS", "NSE_EQ"),
        "NSE_FNO": SessionSpec(_IST, dt_time(9, 15), dt_time(15, 40), "NSE F&O", "NSE_FNO"),
...
def effective_close_time(sess: SessionSpec, now_dt: datetime) -> dt_time:
    """Return the historically correct close for the supplied session date."""
    if sess.segment in {"NSE_EQ", "NSE_FNO"} and now_dt.date() < _CAS_EFFECTIVE_DATE:
        return dt_time(15, 30)
    return sess.close_time
...
def is_open(now_dt: Optional[datetime] = None, segment: Optional[str] = None) -> bool:
    ...
    close_time = effective_close_time(sess, now_dt)
    close_dt = now_dt.replace(hour=close_time.hour, minute=close_time.minute, second=0, microsecond=0)
