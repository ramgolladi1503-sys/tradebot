...
def insert_trade(entry):
    ...
    try:
        with _conn() as conn:
            ...
            (trade_id, timestamp, symbol, underlying, instrument, instrument_type, instrument_token, strike, expiry, option_type, right, instrument_id, side, entry, stop_loss, target, qty, qty_lots, qty_units, validity_sec,
...
def insert_outcome(outcome):
    ...
    try:
        with _conn() as conn:
            conn.execute(
                """
            INSERT INTO outcomes (trade_id, exit_price, exit_time, actual, r_multiple, r_label, timestamp_epoch, timestamp_iso)
            VALUES (?,?,?,?,?,?,?,?)
            """,
                (
