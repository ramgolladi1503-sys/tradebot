...
from core.cross_asset_scoring import cross_asset_adjustment
from core.stock_option_rules import load_stock_option_rules, stock_option_v2_enabled
from core.stock_option_scoring import score_stock_option_shadow_candidate
...
def _build_stock_option_candidates(
    symbol: str,
    rows: list[dict[str, Any]],
    ...
    cross_adj = float(cross_asset_adjustment(symbol, market_data_by_symbol))
    for row in rows:
        ...
        for family in strategy_families:
            score = score_stock_option_shadow_candidate(
                symbol=symbol,
            ...
            confidence = max(0.0, min(1.0, float(score.get("confidence") or 0.0) + cross_adj))
            final_score = max(0.0, min(1.0, float(score.get("final_score") or 0.0) + cross_adj))
            candidates.append(
                {
                    "symbol": symbol,
                    "underlying": symbol,
                    ...
                    "builder_confidence": round(confidence, 4),
                    "gating_final_confidence": round(final_score, 4),
                    ...
                    "source_flags": {
                        ...
                        "spread_ok": True,
                        "fresh_quote_ok": True,
                        "cross_asset_adjustment": round(cross_adj, 4),
                        "score_components": score.get("components", {}),
                    },
                }
            )
    candidates.sort(key=lambda candidate: float(candidate.get("rank_score") or 0.0), reverse=True)
    return candidates
    ...
    *,
    ...
    if stock_option_v2_enabled():
        ...
        for symbol in stock_symbols:
            ...
            stock_candidates = _build_stock_option_candidates(
                symbol,
                selected_rows,
                stock_families,
                ts_epoch,
