...
def build_advisory_latest_payload(limit: int = 200) -> dict[str, Any]:
    ...
    for raw_line in tail_jsonl_rows(path, limit=limit):
        ...
        try:
            ...
            if bool(getattr(cfg, "UI_LIVE_ROW_REQUIRE_TODAY", True)) and not _row_is_today_local(row):
                notes.append(f"stale_row_dropped:{str(row.get('trade_id') or row.get('advisory_id') or 'unknown')}")
                logger.info(
...
__all__ = [
    "build_advisory_latest_payload",
    "build_feed_health_truth_latest_payload",
    "tail_jsonl_rows",
]
