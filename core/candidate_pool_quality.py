...
def analyze_candidate_pool(rows: Iterable[Mapping[str, Any]]) -> CandidatePoolQualityReport:
    items = tuple(dict(row) for row in rows or () if isinstance(row, Mapping))
...
def pool_quality_penalty_for_row(row: Mapping[str, Any], pool: CandidatePoolQualityReport) -> tuple[float, list[str]]:
    ...
    if pool.duplicate_trade_id_count > 0:
        penalty += min(0.10, 0.03 * pool.duplicate_trade_id_count)
...
def _quality_score(
    *,
    ...
    if duplicate_trade_id_count > 0:
        score -= min(0.12, 0.03 * duplicate_trade_id_count)
        reasons.append("duplicate_trade_ids")
...
def _duplicate_trade_id_count(rows: tuple[Mapping[str, Any], ...]) -> int:
    ...
    for row in rows:
        ...
        if not trade_id:
            continue
        groups[trade_id] = groups.get(trade_id, 0) + 1
    return sum(max(count - 1, 0) for count in groups.values())
