...
def build_vectorized_signals(df: pd.DataFrame, config) -> pd.DataFrame:
    ...
    pos_di = 100 * (pos_dm_series.ewm(
        alpha=1 / 14, min_periods=14, adjust=False).mean() / tr_smooth)
    neg_di = 100 * (neg_dm_series.ewm(
        alpha=1 / 14, min_periods=14, adjust=False).mean() / tr_smooth)
    ...
    # Trend VWAP (Breakout of ATR band, strict slope, strong ADX, aligns with
    # Macro Trend)
    ...
    # ORB Breakout (Price crossing the 15-min range + buffer, strong ADX,
    # valid after 09:30)
    ...
    signals_df["signal_side"] = np.where(
        buy_mask, "BUY", np.where(
            sell_mask, "SELL", None))  # type: ignore
    ...
    sig_entry = next_open.loc[signals_df.index].fillna(
        df.loc[signals_df.index, "close"]
    )
    ...
    # Stop and Target must be anchored to the actual execution entry, not the
    # signal bar close
