...
def classify_market_state(snapshot: Mapping[str, Any] | None, *, symbol: str,
                          previous_zone: str | None = None,
                          max_quote_age_sec: float = 5.0) -> MarketStateDecision:
    ...
    if blockers:
        return MarketStateDecision(
            symbol=clean_symbol, zone=NO_TRADE, score=0.0, confidence=0.0,
            entry_state="BLOCKED", bull_trend_price=None,
            bull_reversal_price=None, bear_trend_price=None,
            bear_reversal_price=None, blockers=tuple(blockers),
    ...
    if zone == BEARISH and support is not None and 0 <= price - support <= 0.25 * atr:
        ...
        entry_state = "WAIT_PULLBACK"
    levels = _levels(data, vwap=vwap, atr=atr)
    return MarketStateDecision(
        symbol=clean_symbol, zone=zone, score=score,
        confidence=_confidence(score=score, components=components),
        ...
        diagnostics={
            ...
            "hysteresis": {
                "enter_bull": ENTRY_BULL, "exit_bull": EXIT_BULL,
                "enter_bear": ENTRY_BEAR, "exit_bear": EXIT_BEAR,
            },
        },
    )
...
def classify_cross_index_consensus(states: Mapping[str, MarketStateDecision]) -> dict[str, Any]:
    usable = [state for state in states.values() if not state.blockers]
    if len(usable) < 2:
        return {
            "consensus": NO_TRADE,
            "reason": "INSUFFICIENT_CROSS_INDEX_AUTHORITY",
            "read_only": True,
            "execution_capable": False,
        }
    bull = sum(state.zone == BULLISH for state in usable)
    bear = sum(state.zone == BEARISH for state in usable)
    if bull >= 2 and bear == 0:
        consensus, reason = BULLISH, "CROSS_INDEX_BULL_CONFIRMATION"
    elif bear >= 2 and bull == 0:
        consensus, reason = BEARISH, "CROSS_INDEX_BEAR_CONFIRMATION"
    else:
        consensus, reason = NO_TRADE, "CROSS_INDEX_CONFLICT"
    return {
        "consensus": consensus,
        "reason": reason,
        "members": {key: value.zone for key, value in states.items()},
        "read_only": True,
        "execution_capable": False,
    }
...
def _critical_blockers(data: Mapping[str, Any], *, max_quote_age_sec: float) -> list[str]:
    ...
    if data.get("feed_authority") is not True:
        blockers.append("FEED_AUTHORITY_UNPROVEN" if data.get("feed_authority") is None else "FEED_AUTHORITY_BLOCKED")
    if data.get("session_open") is not True:
        blockers.append("SESSION_AUTHORITY_UNPROVEN" if data.get("session_open") is None else "SESSION_NOT_OPEN")
...
def _breadth_component(data: Mapping[str, Any]) -> float:
    ...
    parts = [_clip(value) for value in (weighted, plain, momentum) if value is not None]
    return sum(parts) / len(parts) if parts else 0.0
...
def _levels(data: Mapping[str, Any], *, vwap: float, atr: float) -> dict[str, float]:
...
def _confidence(*, score: float, components: Mapping[str, float]) -> float:
    directional_agreement = sum(
        1 for value in components.values()
        if (value > 0 and score > 0) or (value < 0 and score < 0)
    )
