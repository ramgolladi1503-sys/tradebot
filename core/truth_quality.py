...
SYNTHETIC_ORIGINS = {"pre_builder_gate", "invalid_snapshot", "softened_builder_path", "softened", "planning_only"}
FALLBACK_ENTRY_SOURCES = {"recovered_fallback", "rest_fallback", "synthetic_offhours", "fallback"}
DEGRADED_REASONS = {"stale_quote", "low_data_confidence", "unverified_spread", "missing_liquidity_validation", "unknown_quote_source", "execution_context_degraded"}
...
def reason_codes(candidate: Any) -> set[str]:
    out: set[str] = set()
    for key in ("reason", "reject_reason", "entry_block_code", "permission_reason", "execution_block_reason", "order_policy_reason", "quote_validation_status"):
...
def derive_truth_quality(candidate: Any) -> str:
    ...
    flags = _flags(candidate)
    origin = str(_get(candidate, "candidate_origin", "") or flags.get("candidate_origin") or flags.get("origin") or flags.get("source") or "").strip().lower()
    ...
    if bool(flags.get("fallback_candidate")) or bool(flags.get("recovered_fallback")) or origin in FALLBACK_ORIGINS or entry_source in FALLBACK_ENTRY_SOURCES or bool(_get(candidate, "phase2_spread_fallback_used", False)) or bool(_get(candidate, "phase2_liquidity_fallback_used", False)):
        return TRUTH_FALLBACK
    if origin in SYNTHETIC_ORIGINS or strategy_family in {"synthetic_advisory", "builder_soft_reject"} or candidate_type == "fallback_market_candidate" or trade_id.startswith(("softrej_", "tbsoft_")) or bool(flags.get("recoverable_soft_reject")):
    ...
    if quote_source in {"", "unknown", "none"} or reason_codes(candidate) & DEGRADED_REASONS:
...
def apply_truth_quality_contract(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    if cap == "ADVISORY_ONLY":
        out.update({"permission": "ADVISORY_ONLY", "final_action": "ADVISORY_ONLY", "execution_status": "advisory_only", "candidate_status": "advisory_only", "execution_allowed": False, "eligible_for_execution": False, "tradable": False, "is_executable": False, "truth_block_reason": f"truth_quality_{truth_quality.lower()}"})
    elif cap == "QUEUE_ONLY" and str(out.get("final_action") or "").strip().upper() == "EXECUTE":
        out.update({"permission": "QUEUE_ONLY", "final_action": "QUEUE_ONLY", "execution_status": "queue_only", "execution_allowed": False, "eligible_for_execution": False, "is_executable": False, "truth_block_reason": "truth_quality_degraded"})
    return out
