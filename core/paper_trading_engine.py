...
from core.paper_capital_controller import PaperCapitalController
...
class PaperTradingEngine:
    def __init__(self) -> None:
        ...
        self.capital_controller = PaperCapitalController()

    def enter_position(self, candidate: dict[str, Any], now_ts: float) -> PaperPosition | None:
        allowed, reason = self.capital_controller.can_open_trade()
        if not allowed:
            return None
        ...
        base_qty = int(candidate.get("qty") or candidate.get("qty_units") or 1)
        qty = self.capital_controller.apply_quantity_multiplier(base_qty)
    ...
    def exit_position(self, trade_id: str, exit_price: float, now_ts: float, *, reason: str = "manual") -> PaperPosition | None:
        ...
        self.capital_controller.record_closed_trade(position.realized_pnl)
    ...
    def snapshot(self) -> PaperTradingSnapshot:
        ...
        return PaperTradingSnapshot(
            ...
            capital_state=self.capital_controller.state().__dict__,
        )
