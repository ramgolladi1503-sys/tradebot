...
_TRACE_ENV = "TRADEBOT_FEED_FD_TRACE"
_TRACE_PATH_ENV = "TRADEBOT_FEED_FD_TRACE_PATH"
_TRACE_EVERY_N_ENV = "TRADEBOT_FEED_FD_TRACE_EVERY_N"
_TRACE_DELTA_ENV = "TRADEBOT_FEED_FD_TRACE_DELTA"
_TRACE_HIGH_ENV = "TRADEBOT_FEED_FD_TRACE_HIGH"

TRACE_EVERY_N = max(1, int(os.environ.get(_TRACE_EVERY_N_ENV, "1000")))
TRACE_FD_DELTA_THRESHOLD = max(1, int(os.environ.get(_TRACE_DELTA_ENV, "5")))
TRACE_FD_BASELINE_HIGH = max(1, int(os.environ.get(_TRACE_HIGH_ENV, "20")))
TRACE_ENABLED = str(os.environ.get(_TRACE_ENV, "")).strip().lower() in {"1", "true", "yes", "on"}

_TRACE_LOCK = threading.Lock()
_TRACE_COUNTER = 0
_TRACE_BASELINE_FD: int | None = None
_TRACE_LAST_FD: int | None = None
_TRACE_PATH = Path(os.environ.get(_TRACE_PATH_ENV, "")).expanduser() if os.environ.get(_TRACE_PATH_ENV) else None
...
def logger_handler_count() -> int:
    ...
    return len(logger_handler_inventory()) + len(list(root.handlers or []))
...
def should_sample(*, row_index: int | None, fd_count: int | None, baseline_fd: int | None = None) -> bool:
    ...
    if fd_count is not None and baseline is not None and fd_count - baseline >= TRACE_FD_DELTA_THRESHOLD:
        return True
    return False
...
def record_trace(
    ...
    *,
    ...
    if not TRACE_ENABLED:
        return None
    if getattr(_TRACE_LOCAL, "active", False):
        return None
    ...
    with _TRACE_LOCK:
        ...
        if event.fd_count is not None and baseline is not None and event.fd_count >= baseline + TRACE_FD_BASELINE_HIGH:
