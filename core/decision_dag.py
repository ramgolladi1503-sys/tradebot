...
def _node_strategy_select(snapshot: MarketSnapshot, ctx: Mapping[str, Any], deps: Mapping[str, NodeResult]) -> NodeResult:
    ...
    facts: dict[str, Any] = {
        ...
        "indicator_source": snapshot.raw_data.get("indicator_source"),
        "indicator_readiness_source": snapshot.raw_data.get("indicator_readiness_source"),
        "indicator_last_update_epoch": snapshot.indicator_last_update_epoch,
        "indicator_age_sec": snapshot.indicators_age_sec,
        "indicators_ok": snapshot.indicators_ok,
        "indicator_missing_inputs": list(snapshot.raw_data.get("indicator_missing_inputs") or []),
        "indicator_readiness_ready": snapshot.raw_data.get("indicator_readiness_ready"),
        "indicator_readiness_reason": snapshot.raw_data.get("indicator_readiness_reason"),
        "indicator_readiness_blockers": list(snapshot.raw_data.get("indicator_readiness_blockers") or []),
...
def _node_final_decision(snapshot: MarketSnapshot, ctx: Mapping[str, Any], deps: Mapping[str, NodeResult]) -> NodeResult:
    ...
    if snapshot.mode == "LIVE" and ("session_state" in snapshot.raw_data) and snapshot.session_state != SESSION_NORMAL_OPEN:
        allowed = False
