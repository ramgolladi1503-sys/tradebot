"""Pure VWAP CandidateIntent generator for EDGE-73."""
...
VWAP_CANDIDATE_GENERATOR_SCHEMA_VERSION = 1
VWAP_CANDIDATE_GENERATOR_SOURCE = "vwap_candidate_generator_v1"

VWAP_MISSING_MARKET_STATE = "vwap_missing_market_state"
VWAP_MISSING_INSTRUMENT = "vwap_missing_instrument"
VWAP_MISSING_LTP = "vwap_missing_ltp"
VWAP_MISSING_VWAP = "vwap_missing_vwap"
VWAP_INVALID_NUMERIC_INPUT = "vwap_invalid_numeric_input"
VWAP_INVALID_PARAMETER = "vwap_invalid_parameter"
VWAP_NO_TREND_CONFIRMATION = "vwap_no_trend_confirmation"
VWAP_SLOPE_NOT_CONFIRMED = "vwap_slope_not_confirmed"
VWAP_DEVIATION_TOO_SMALL = "vwap_deviation_too_small"
...
_VWAP_KEYS = ("vwap", "session_vwap", "anchored_vwap")
_SLOPE_KEYS = ("vwap_slope", "slope", "vwap_trend")
_REGIME_KEYS = ("regime", "market_regime", "regime_state")
_INSTRUMENT_KEYS = ("instrument", "symbol", "tradingsymbol", "underlying")


@dataclass(frozen=True)
class VwapCandidateGenerationReport:
    schema_version: int
    read_only: bool
    append: bool
    source: str
...
def build_vwap_candidate_intents(
    market_state: Mapping[str, Any] | None,
    *,
    instrument: str | None = None,
    strategy_id: str = "vwap_v1",
    min_deviation_bps: float = 15.0,
    min_slope: float = 0.0,
    source: str = VWAP_CANDIDATE_GENERATOR_SOURCE,
) -> VwapCandidateGenerationReport:
    """Build a read-only VWAP CandidateIntent from a market-state snapshot."""

    payload = dict(market_state or {}) if isinstance(market_state, Mapping) else {}
    blockers: list[str] = []
    warnings: list[str] = []
    if not isinstance(market_state, Mapping):
        blockers.append(VWAP_MISSING_MARKET_STATE)

    min_deviation_bps_value = _threshold(min_deviation_bps, allow_zero=False)
    min_slope_value = _threshold(min_slope, allow_zero=True)
    if _INVALID_FLOAT in (min_deviation_bps_value, min_slope_value):
        blockers.append(VWAP_INVALID_PARAMETER)
    ...
    if not resolved_instrument:
        ...
        blockers.append(VWAP_MISSING_INSTRUMENT)

    ltp = _first_float(payload, _LTP_KEYS)
    vwap = _first_float(payload, _VWAP_KEYS)
    slope = _first_float(payload, _SLOPE_KEYS, default=0.0)
    numeric_invalid = any(item is _INVALID_FLOAT for item in (ltp, vwap, slope))
    if numeric_invalid:
        blockers.append(VWAP_INVALID_NUMERIC_INPUT)
    if ltp in (None, _INVALID_FLOAT):
        blockers.append(VWAP_MISSING_LTP)
    if vwap in (None, _INVALID_FLOAT):
        blockers.append(VWAP_MISSING_VWAP)

    direction = "NO_TRADE"
    trigger = "vwap_conditions_not_confirmed"
    invalidation = "ltp_returns_to_vwap_neutral_zone"
    intent_type = INTENT_TYPE_NO_TRADE
    deviation_bps = None
    if not blockers:
        deviation_bps = ((float(ltp) - float(vwap)) / float(vwap)) * 10000.0
        if abs(deviation_bps) < float(min_deviation_bps_value):
            blockers.append(VWAP_DEVIATION_TOO_SMALL)
        elif deviation_bps > 0:
            direction = "BUY_CALL"
            trigger = "ltp_above_vwap_with_trend_confirmation"
            if float(slope) < float(min_slope_value):
                blockers.append(VWAP_SLOPE_NOT_CONFIRMED)
                warnings.append("vwap_uptrend_blocked_by_slope")
        elif deviation_bps < 0:
            direction = "BUY_PUT"
            trigger = "ltp_below_vwap_with_trend_confirmation"
            if float(slope) > -float(min_slope_value):
                blockers.append(VWAP_SLOPE_NOT_CONFIRMED)
                warnings.append("vwap_downtrend_blocked_by_slope")
        else:
            blockers.append(VWAP_NO_TREND_CONFIRMATION)
    ...
    intent = create_candidate_intent(
        ...
        metadata={
            "adapter_source": VWAP_CANDIDATE_GENERATOR_SOURCE,
            "input_keys": sorted(str(key) for key in payload.keys()),
            "vwap_position": _vwap_position(ltp, vwap, deviation_bps, min_deviation_bps_value),
            "deviation_bps": _safe_number(deviation_bps),
            "vwap_slope": _safe_number(slope),
            "min_deviation_bps": _safe_number(min_deviation_bps_value),
            "min_slope": _safe_number(min_slope_value),
            "does_not_rank_candidates": True,
            "does_not_score_edge": True,
        },
    )
    pool_report = build_candidate_intent_pool((intent,))
    return VwapCandidateGenerationReport(
        schema_version=VWAP_CANDIDATE_GENERATOR_SCHEMA_VERSION,
        read_only=True,
        append=False,
        source=source,
...
def _vwap_position(ltp: Any, vwap: Any, deviation_bps: Any, min_deviation_bps: Any) -> str:
    if any(value in (None, _INVALID_FLOAT) for value in (ltp, vwap, min_deviation_bps)):
        return "UNKNOWN"
...
def _metadata() -> dict[str, Any]:
    return {
        "model": VWAP_CANDIDATE_GENERATOR_SOURCE,
        "scope": "pure_vwap_candidate_intent_generator_only",
...
__all__ = [
    "VWAP_CANDIDATE_GENERATOR_SCHEMA_VERSION",
    "VWAP_CANDIDATE_GENERATOR_SOURCE",
