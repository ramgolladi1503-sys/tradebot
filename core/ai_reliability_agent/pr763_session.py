...
def verify_sealed_evidence_root(root: str | Path) -> Gate:
    ...
    if errors:
        return Gate("SEALED_EVIDENCE_ROOT", False, {"root": str(evidence_root), "errors": errors})
    ...
    try:
        ...
        return Gate(
            ...
            False,
            {"root": str(evidence_root), "errors": [f"metadata_parse_error:{type(exc).__name__}"]},
        )
    ...
    actual_files = {
        ...
        if path.is_file() and path.name not in {"artifact_manifest.json", "SHA256SUMS", "SEALED"}
    }
            ...
            with path.open("r", encoding="utf-8") as handle:
                for line_number, raw in enumerate(handle, start=1):
                    ...
                    try:
                        ...
                        yield {"_parse_error": line_number, "_source_path": str(path)}
                        continue
...
def _artifact_json_records(root: Path) -> Iterator[dict[str, Any]]:
    for path in sorted(root.rglob("*")):
        if path.is_file() and path.suffix.lower() in {".json", ".jsonl"} and path.name not in {
...
def verify_read_only_evidence(root: str | Path) -> Gate:
    ...
    for row in _artifact_json_records(Path(root)):
        ...
        if "broker_api_called" in row and _explicit_true(row.get("broker_api_called")):
    ...
    return Gate(
        ...
        not errors,
        {"records_scanned": records, "parse_errors": parse_errors, "violations": sorted(set(violations)), "errors": errors},
    )
...
def _collect_authority_rows(value: Any, *, container: str = "") -> Iterator[tuple[dict[str, Any], str]]:
    if isinstance(value, Mapping):
        row = dict(value)
        authority_keys = {"authority_state", "authority_allowed", "operator_bucket", "canonical_execution_decision"}
        if authority_keys.intersection(row):
            yield row, container
        for key, child in value.items():
            next_container = str(key) if key in {
                "top_executable",
                "top_advisory",
                "advisory",
                "blocked_debug",
...
def verify_authority_snapshots(snapshot_paths: Sequence[str | Path]) -> Gate:
    ...
    for raw_path in snapshot_paths:
        ...
        try:
            ...
            errors.append(f"snapshot_parse_error:{path}:{type(exc).__name__}")
            continue
        found = list(_collect_authority_rows(payload))
        if not found and isinstance(payload, Mapping) and any(
            key in payload for key in ("top_executable", "top_advisory", "advisory", "blocked_debug")
        ):
    ...
    for index, (row, container, source) in enumerate(rows):
        ...
        fallback = any(
            ...
            for key in ("fallback_used", "recovered_fallback", "synthetic", "is_synthetic", "stale_quote", "quote_is_stale")
        )
        quote_source = str(row.get("quote_source") or row.get("option_ltp_source") or "").strip().upper()
        unsafe_quote_source = (not quote_source) or any(
            token in quote_source for token in ("FALLBACK", "SYNTHETIC", "UNKNOWN", "MOCK", "NONE")
        )
        if state == "EXECUTABLE" or allowed or bucket == "TOP_EXECUTABLE" or container == "top_executable":
            executable_count += 1
            ...
            if selection_score <= 0.0:
                errors.append(f"executable_selection_score_not_positive:{row_id}:{source}")
            if fallback or unsafe_quote_source:
                errors.append(f"unsafe_quote_in_executable:{row_id}:{source}")
            if any(field in row and _explicit_false(row.get(field)) for field in _EXECUTION_FLAGS[:5]):
                errors.append(f"executable_flag_false:{row_id}:{source}")
        else:
            non_executable_ids.add(row_id)
            if bucket == "ADVISORY_ONLY" or container in {"top_advisory", "advisory"}:
                advisory_count += 1
            else:
                blocked_count += 1
            ...
            if abs(selection_score) > 1e-12:
                errors.append(f"non_executable_selection_score_nonzero:{row_id}:{source}")
            for field in _CAPITAL_FIELDS:
                if field in row and abs(_safe_float(row.get(field))) > 1e-12:
                    errors.append(f"non_executable_capital_nonzero:{row_id}:{field}:{source}")
            for field in _EXECUTION_FLAGS:
                if field in row and _explicit_true(row.get(field)):
                    errors.append(f"non_executable_flag_true:{row_id}:{field}:{source}")
    duplicates = sorted(executable_ids.intersection(non_executable_ids))
    if duplicates:
        errors.append(f"identity_in_executable_and_non_executable:{','.join(duplicates)}")
    if snapshots == 0:
...
def discover_live_semantics(root: str | Path) -> Gate:
    ...
    for row in _artifact_json_records(Path(root)):
        ...
        if row.get("_parse_error"):
            continue
        proof_kind = str(row.get("proof_kind") or row.get("evidence_kind") or "").strip().upper()
        symbol = str(row.get("symbol") or row.get("underlying") or "").strip().upper()
        packet_mode = str(row.get("packet_mode") or row.get("mode") or row.get("kite_mode") or "").strip().upper()
        if (
            ...
            or (symbol == "NIFTY" and packet_mode == "FULL" and _explicit_true(row.get("packet_received")))
            or (proof_kind == "PR763_LIVE_ACCEPTANCE" and _explicit_true(row.get("post_mode_full_nifty_packets")))
        ):
        ...
        if (
            (isinstance(completed_bars, list) and len(completed_bars) > 0)
            or _safe_float(row.get("completed_constituent_bar_count")) > 0
            or _safe_float(row.get("constituent_bar_count")) > 0
            or (proof_kind == "PR763_LIVE_ACCEPTANCE" and _safe_float(completed_bars) > 0)
        ):
        ...
        if (
            ...
            or (proof_kind == "PR763_LIVE_ACCEPTANCE" and _explicit_true(row.get("market_event_graph_traversal")))
        ):
    ...
    return Gate(
        ...
        {
            "proof": proof,
            "observations": {key: sorted(set(paths)) for key, paths in observations.items()},
            "missing": missing,
        },
    )
    ...
    *,
    ...
    report = {
        **semantic,
        ...
        "authority_snapshot_paths": [str(Path(path)) for path in authority_snapshot_paths],
    }
...
def render_markdown(report: Mapping[str, Any]) -> str:
    ...
    for gate in report.get("gates") or []:
        lines.append(f"- {'PASS' if gate.get('passed') else 'FAIL'} `{gate.get('gate_id')}`")
