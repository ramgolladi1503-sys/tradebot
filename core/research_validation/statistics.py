...
def probabilistic_sharpe_ratio(returns: Iterable[float], *, benchmark_sharpe: float = 0.0, max_lag: int | None = None) -> float:
    ...
    if variance_term <= 0.0 or not math.isfinite(variance_term):
        raise ValueError("estimated Sharpe variance is non-positive or non-finite")
...
def effective_rank(correlation_matrix: Sequence[Sequence[float]]) -> float:
    ...
    p = eig[eig > _EPS] / float(np.sum(eig))
...
def deflated_sharpe_ratio(selected_returns: Iterable[float], trial_sharpes: Iterable[float], *, effective_trials: float | None = None, max_lag: int | None = None) -> float:
    ...
    if effective_trials < 1.0 or effective_trials > trials.size:
        raise ValueError("effective_trials must be within [1, raw trial count]")
    trial_sd = float(np.std(trials, ddof=1))
    if trial_sd <= _EPS:
        raise ValueError("trial Sharpe distribution must have non-zero variance")
...
def _finite_p_values(p_values: Iterable[float]) -> list[float]:
    ...
    if any(isinstance(v, bool) or not isinstance(v, (int, float)) for v in raw):
        raise ValueError("p-values must be real numbers, not booleans or strings")
...
def _column_sharpes(matrix: np.ndarray) -> np.ndarray:
    ...
    scores = np.divide(means, sds, out=np.full_like(means, np.nan), where=sds > _EPS)
    return scores
...
def cscv_probability_of_backtest_overfitting(strategy_returns: Sequence[Sequence[float]], *, blocks: int = 8) -> float:
    ...
    if type(blocks) is not int or blocks < 4 or blocks % 2:
    ...
    for train_blocks in itertools.combinations(range(blocks), half):
        if 0 not in train_blocks:
            continue
        test_blocks = [i for i in range(blocks) if i not in set(train_blocks)]
        ...
        if not np.isfinite(train_scores).any():
            raise ValueError("all train strategy Sharpes are undefined in a CSCV split")
        winner = int(np.nanargmax(train_scores))
        test_scores = _column_sharpes(matrix[test_idx, :])
        if not np.isfinite(test_scores[winner]):
            raise ValueError("selected strategy Sharpe is undefined out of sample")
        finite_scores = test_scores[np.isfinite(test_scores)]
        rank = 1 + int(np.sum(finite_scores < test_scores[winner]))
        omega = rank / (finite_scores.size + 1.0)
        if omega <= 0.5:
            lower_half_count += 1
        comparisons += 1
    if comparisons == 0:
