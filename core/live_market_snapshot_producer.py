...
def build_live_market_snapshot(*, db_path: str | Path, output_path: str | Path,
    ...
    index_tokens = _index_tokens(instrument_path)
    ...
    try:
        ...
        for symbol, token in index_tokens.items():
            ...
            minute_rows = conn.execute(
                ...
                (token, ts - 60.0, ts, str(session_date) + "%"),
            ).fetchall()
            symbols[symbol] = {
    ...
    missing_symbols = sorted(set({"NIFTY", "BANKNIFTY", "SENSEX"}) - set(index_tokens))
    warnings = ["option_surface_unavailable", "cross_asset_unavailable"]
    if missing_symbols:
        warnings.append("index_instrument_missing:" + ",".join(missing_symbols))
    ...
    snapshot["producer_meta"].update({"session_id": session_id, "session_date": session_date, "source_sha": source_sha, "max_input_timestamp": _iso(max_input), "input_sources": [str(Path(db_path).resolve()), str(Path(instrument_path).resolve())], "causal_cutoff": _iso(max_input), "feature_availability": feature_availability, "vwap_status": "UNAVAILABLE_UNTRUSTWORTHY_VOLUME"})
