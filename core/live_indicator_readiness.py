...
def build_live_indicator_readiness_report(
    ...
    *,
    ...
    safe_warmup = max(0, int(warmup_min_bars))
    safe_max_age = max(0.0, float(max_indicator_age_sec))
    if not snapshots:
        return LiveIndicatorReadinessReport(
            ...
            warnings=(),
            metadata=_metadata(warmup_min_bars=safe_warmup, max_indicator_age_sec=safe_max_age),
    ...
    decisions = tuple(
        _decision_for_snapshot(
            snapshot,
            now_epoch=now_value,
            warmup_min_bars=safe_warmup,
            max_indicator_age_sec=safe_max_age,
            source=source,
        )
