...
from mros_program_catalog import sprint_spec,next_sprint
...
def test_review_and_audit_wait_automatically():
    state={'active_milestone':'M1','active_sprint':'S004'};req=['x/requests/S004_R001_R01.json'];assert derive_phase(state,req,{})==('REVIEW_RUNNING','WAIT_AUTOMATICALLY')
    req=['x/requests/S004_A001_A01.json'];assert derive_phase(state,req,{})==('AUDIT_RUNNING','WAIT_AUTOMATICALLY')
...
def test_no_pending_work_routes_to_correct_autonomous_cycle():
    assert derive_phase({'active_milestone':'M1','active_sprint':'S003'},[],{})==('AUTONOMOUS_S003_CYCLE','RUN_AUTONOMOUS_CYCLE')
    assert derive_phase({'active_milestone':'M1','active_sprint':'S004'},[],{})==('AUTONOMOUS_PROGRAM_CYCLE','RUN_AUTONOMOUS_CYCLE')
    assert derive_phase({'active_milestone':'M8','active_sprint':'S110'},[],{})==('AUTONOMOUS_PROGRAM_CYCLE','RUN_AUTONOMOUS_CYCLE')

def test_m9_and_completed_m8_are_hard_stops():
    assert derive_phase({'active_milestone':'M9','active_sprint':'S111'},[],{})==('HARD_STOP','M9_BOUNDARY_PRESERVED')
    assert derive_phase({'active_milestone':'M8','active_sprint':'S110','program_status':'M8_COMPLETE_M9_HARD_STOP'},[],{})==('HARD_STOP','M9_BOUNDARY_PRESERVED')
...
def test_blocking_findings_include_invalid_artifact_findings():
    f={'severity':'MAJOR','requirement':'bind receipt path','evidence':'x'};aggregate={'reviews':[],'invalid':[{'review':{'findings':[f]}}]};assert blocking_findings(aggregate,'review')==[f]

def test_program_catalog_boundaries_and_assurance():
    assert sprint_spec(4).phase=='VERIFICATION_CALIBRATION_INDEPENDENT_ATTACK'
    assert sprint_spec(30).assurance_tier=='FULL' and sprint_spec(30).milestone=='M1'
    assert sprint_spec(110).milestone=='M8' and sprint_spec(110).terminal_m8 is True and next_sprint(110) is None
    with pytest.raises(ValueError):sprint_spec(111)

def test_auxiliary_controller_modules_are_importable():
    assert callable(finalizer.finalize);assert callable(calibration_repair.main);assert callable(post_cycle.cycle);assert callable(sprint_executor.execute);assert callable(program_repair.execute);assert callable(program_native.validate)
