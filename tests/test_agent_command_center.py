...
def test_command_center_renders_candidate_supply_zero_attribution_markdown(tmp_path: Path):
    runtime_dir, logs_dir = _write_runtime(
        tmp_path,
        feed_runtime={
            "run_id": "run-current",
    ...
    md = (runtime_dir / "agent_reports" / "agent_command_center_latest.md").read_text(encoding="utf-8")
    payload = report.to_dict()
    assert payload["metrics_summary"]["first_current_session_blocker"] == "CANDIDATE_SUPPLY"
    assert "## Candidate Supply Zero Attribution" in md
