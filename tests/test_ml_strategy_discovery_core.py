...
def synthetic_bars(days: int = 8, bars_per_day: int = 120) -> pd.DataFrame:
    ...
    for day in range(days):
        start = pd.Timestamp("2026-01-05 03:45:00", tz="UTC") + pd.Timedelta(days=day)
        ...
        for bar in range(bars_per_day):
            ...
            volume = 1000 + 15 * bar + (1200 if bar < 20 else 0) + rng.integers(0, 500)
            rows.append(
                {
...
def test_dataset_is_deterministic_and_marks_missing_option_data() -> None:
    ...
    second = build_discovery_dataset(bars.sample(frac=1.0, random_state=4), config=config())
...
def test_chronological_split_locks_holdout() -> None:
    ...
    dev_end = split.loc[split["split"] == "DEVELOPMENT", "decision_timestamp"].max()
...
def test_training_does_not_use_locked_holdout() -> None:
    ...
    artifacts = train_discovery_models(
        split, config=config(), max_tree_depth=3, minimum_leaf_rows=10
    )
    ...
    second = train_discovery_models(
        mutated, config=config(), max_tree_depth=3, minimum_leaf_rows=10
    )

    assert artifacts.validation_metrics == second.validation_metrics
    assert [c.candidate_id for c in artifacts.candidates] == [
        c.candidate_id for c in second.candidates
    ]
    assert artifacts.validation_metrics["holdout_rows_not_read"] == int(holdout.sum())


def test_candidate_evaluation_controls_and_holdout_guard() -> None:
    ...
    with pytest.raises(PermissionError):
        evaluate_locked_holdout_once(
            dataset, candidate, acknowledgement="NO"
        )
