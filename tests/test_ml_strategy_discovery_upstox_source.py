...
def _write_certified_session(
    ...
    *,
    ...
    base = 22000.0 + np.linspace(0.0, 90.0, len(timestamps))
    close = base + np.sin(np.arange(len(timestamps)) / 7.0)
    frame = pd.DataFrame(
        {
...
def test_certified_adapter_rejects_incomplete_session(tmp_path: Path) -> None:
    ...
    payload["records"][0]["row_count"] = len(frame)
    manifest_path.write_text(json.dumps(payload), encoding="utf-8")
