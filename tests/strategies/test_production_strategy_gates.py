...
def test_vwap_orb_fails_closed_on_positive_gamma():
    # Proves that positive gamma suppresses volatility
    # and vetoes ORB trades
    assert (
        vwap_orb_strategy(
            "NIFTY",
            101.0,
...
def test_zero_hero_requires_expiry_context(monkeypatch):
    # Proves the strategy respects the strict expiry config
    # when non-expiry is disabled
...
def test_volatility_trend_fails_closed_on_no_cross_asset_confirmation():
    # Proves that it vetoes if cross-assets are not
    ...
    assert (
        volatility_scaled_trend_strategy(
            ...
            100.0,
            1.2,
            cross_assets={"BANKNIFTY": {"ltp": 90.0, "vwap": 100.0}},
        )
        == []
    )
