...
def test_blocked_epoch_is_not_sealed_or_opened():
    ...
    assert final["FINAL_VERDICT"] == "BLOCKED_HISTORICAL_CREDENTIAL_UNAVAILABLE"
    assert final["data_epoch_sealed"] is False
    assert final["holdout_opened"] is False
    assert final["candidate_logic_run"] is False
    assert final["outcomes_calculated"] is False
