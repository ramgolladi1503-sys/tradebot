    ...
    metadata = {
        **frozen_threshold_metadata(),
        ...
        "completed_constituent_bars": [
            {"ts_epoch": 100.0, "source_bar_end_epoch": 90.0, "session_date": "2026-07-29", "index_ret1": -0.001, "constituent_ret1": _returns(40), "completed": True},
            {"ts_epoch": 160.0, "source_bar_end_epoch": 150.0, "session_date": "2026-07-29", "index_ret1": -0.004, "constituent_ret1": _returns(25), "completed": True},
            {"ts_epoch": 220.0, "source_bar_end_epoch": 210.0, "session_date": "2026-07-29", "index_ret1": 0.001, "constituent_ret1": _returns(5), "completed": True},
            {"ts_epoch": 280.0, "source_bar_end_epoch": 270.0, "session_date": "2026-07-29", "index_ret1": 0.001, "constituent_ret1": _returns(20), "completed": True},
        ],
    }
    return metadata
...
def test_threshold_boundaries_are_inclusive_without_override_metadata():
    metadata = {
        **frozen_threshold_metadata(),
        "market_event_graph_runtime_state": initial_market_event_graph_runtime_state("2026-07-29"),
        "completed_constituent_bars": [
            {"ts_epoch": 100.0, "source_bar_end_epoch": 90.0, "session_date": "2026-07-29", "index_ret1": 0.0, "constituent_ret1": _returns(54, total=247), "completed": True},
            {"ts_epoch": 160.0, "source_bar_end_epoch": 150.0, "session_date": "2026-07-29", "index_ret1": FROZEN_THRESHOLDS["divergence_low"], "constituent_ret1": [0.0] * 247, "completed": True},
            {"ts_epoch": 220.0, "source_bar_end_epoch": 210.0, "session_date": "2026-07-29", "index_ret1": 0.0, "constituent_ret1": _returns(25, total=247), "completed": True},
            {"ts_epoch": 280.0, "source_bar_end_epoch": 270.0, "session_date": "2026-07-29", "index_ret1": 0.0, "constituent_ret1": _returns(100, total=247), "completed": True},
        ],
    }
...
def test_strategy_can_emit_from_raw_completed_constituent_bars():
    ...
    assert candidates[0].evidence["idempotency_key"] == candidates[0].evidence["triplet_id"]
    assert metadata["market_event_graph_runtime_state"]["last_processed_entry_bar_ts_epoch"] == 280.0
    assert metadata["market_event_graph_runtime_state"]["last_emitted_triplet_id"] == candidates[0].evidence["triplet_id"]


def test_strategy_idempotency_uses_persisted_runtime_state():
    metadata = _metadata()
    ctx = StrategyContext(symbol="NIFTY", ts_epoch=280.0, option_ce_ltp=120.0, option_pe_ltp=105.0, ce_premium_change=2.0, pe_premium_change=-1.0, ce_spread_pct=0.01, pe_spread_pct=0.01, ce_depth=1500.0, pe_depth=1300.0, quote_source="realtime", fallback_used=False, option_ltp_age_sec=1.0, metadata=metadata)
    assert len(generate_market_event_graph_reversal_candidates(ctx, _regime())) == 1
    assert generate_market_event_graph_reversal_candidates(ctx, _regime()) == ()
    fresh_context = StrategyContext(symbol="NIFTY", ts_epoch=280.0, option_ce_ltp=120.0, option_pe_ltp=105.0, ce_premium_change=2.0, pe_premium_change=-1.0, ce_spread_pct=0.01, pe_spread_pct=0.01, ce_depth=1500.0, pe_depth=1300.0, quote_source="realtime", fallback_used=False, option_ltp_age_sec=1.0, metadata=metadata)
    assert generate_market_event_graph_reversal_candidates(fresh_context, _regime()) == ()
    metadata["completed_constituent_bars"].append({"ts_epoch": 340.0, "source_bar_end_epoch": 330.0, "session_date": "2026-07-29", "index_ret1": 0.001, "constituent_ret1": _returns(20), "completed": True})
    assert generate_market_event_graph_reversal_candidates(fresh_context, _regime()) == ()
...
def test_distinct_later_graph_emits_once_after_state_watermark():
    ...
    metadata["completed_constituent_bars"].extend([
        {"ts_epoch": 340.0, "source_bar_end_epoch": 330.0, "session_date": "2026-07-29", "index_ret1": -0.001, "constituent_ret1": _returns(40), "completed": True},
        {"ts_epoch": 400.0, "source_bar_end_epoch": 390.0, "session_date": "2026-07-29", "index_ret1": -0.004, "constituent_ret1": _returns(25), "completed": True},
        {"ts_epoch": 460.0, "source_bar_end_epoch": 450.0, "session_date": "2026-07-29", "index_ret1": 0.001, "constituent_ret1": _returns(5), "completed": True},
        {"ts_epoch": 520.0, "source_bar_end_epoch": 510.0, "session_date": "2026-07-29", "index_ret1": 0.001, "constituent_ret1": _returns(20), "completed": True},
    ])
    later = StrategyContext(symbol="NIFTY", ts_epoch=520.0, option_ce_ltp=120.0, option_pe_ltp=105.0, ce_premium_change=2.0, pe_premium_change=-1.0, ce_spread_pct=0.01, pe_spread_pct=0.01, ce_depth=1500.0, pe_depth=1300.0, quote_source="realtime", fallback_used=False, option_ltp_age_sec=1.0, metadata=metadata)
    assert len(generate_market_event_graph_reversal_candidates(later, _regime())) == 1
    assert generate_market_event_graph_reversal_candidates(later, _regime()) == ()
