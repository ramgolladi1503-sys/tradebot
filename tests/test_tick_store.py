...
def test_async_insert_does_not_flush_sqlite_on_callback_thread(monkeypatch, tmp_path):
    ...
    callback_flushes_before_shutdown = []
    original_flush_pending_ticks = tick_store._flush_pending_ticks
    shutdown_started = False

    def _record_flush(*args, **kwargs):
        if not shutdown_started and threading.get_ident() == callback_thread_ident:
            callback_flushes_before_shutdown.append((args, kwargs))
    ...
    with monkeypatch.context() as scoped:
        ...
        # The safety contract is specifically that callback/direct insertion does
        # not synchronously flush SQLite. Governed shutdown is allowed to perform
        # its own final drain on the caller thread, so exclude that phase from the
        # callback-thread assertion while keeping the probe installed until the
        # worker is joined.
        shutdown_started = True
        tick_store.shutdown_persistence_worker(deadline_seconds=1.0)

    assert callback_flushes_before_shutdown == []
