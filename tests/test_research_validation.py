...
def test_reference_sharpe_numbers_match_public_2026_examples():
    ...
    assert round(
        minimum_track_record_length(sr, 0, skewness=-2.448, kurtosis=10.164), 3
    ) == 13.029
    assert round(
        probabilistic_sharpe_ratio(sr, 0, 24, skewness=-2.448, kurtosis=10.164), 3
    ) == 0.987
    assert round(
        1 - sharpe_ratio_power(0, 0.5, 24, skewness=-2.448, kurtosis=10.164), 3
    ) == 0.315
...
def test_effective_rank_distinguishes_independent_and_correlated_trials():
    ...
    returns = np.column_stack(
        [
            base[:, 0],
            base[:, 0] + 0.01 * rng.normal(size=500),
            base[:, 1],
            base[:, 1] + 0.01 * rng.normal(size=500),
        ]
    )
...
def test_multiple_testing_adjustments_are_bounded_and_fwer_fdr_distinct():
    ...
    for adjusted in (bonf, sidak, holm, bh, by):
...
def test_trial_ledger_is_hash_bound_and_fail_closed_on_missing_parent():
    rows = [
        TrialRecord(
            "T1", "H1", "S1", "abc123", "data123", {"x": 1}, "REJECTED", False, {"sr": 0.1}
        ),
        TrialRecord(
            "T2",
            "H1",
...
def test_distribution_gate_warns_without_claiming_population_moment_nonexistence():
