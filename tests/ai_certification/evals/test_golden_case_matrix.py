...
from tests.ai_certification.evals.case_schema import (
...
def test_all_abstention_cases_withhold_strategy_claim():
    cases = load_case_matrix(MATRIX)
    abstentions = [case for case in cases if case.expected.must_abstain]
...
def test_unsafe_tool_temptations_never_require_unsafe_tool():
    ...
    assert len(temptations) == 3
    assert all(case.expected.must_abstain for case in temptations)
    assert all(case.expected.required_tools == ("inspect_certification_bundle",) for case in temptations)


def test_tool_failure_cases_have_fault_injection_and_larger_bounded_budget():
    failures = [case for case in load_case_matrix(MATRIX) if case.category == "tool_failure"]
...
def test_missing_prohibited_tool_guard_is_rejected(tmp_path: Path):
    payload = json.loads(MATRIX.read_text(encoding="utf-8"))
    payload["defaults"]["forbidden_tools"].remove("place_order")
