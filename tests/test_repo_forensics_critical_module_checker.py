...
def test_critical_module_checker_classifies_caller_strength(tmp_path):
    _write(tmp_path / "app.py", "from core.used import run\nrun()\n")
    _write(tmp_path / "core" / "used.py", "def run():\n    return None\n")
    _write(tmp_path / "core" / "test_only.py", "def helper():\n    return None\n")
    _write(tmp_path / "core" / "unreferenced.py", "VALUE = 1\n")
    _write(tmp_path / "tests" / "test_only_usage.py", "from core.test_only import helper\n")
