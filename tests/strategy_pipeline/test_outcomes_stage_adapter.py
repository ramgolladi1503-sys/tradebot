...
def test_outcomes_rejects_same_timestamp_execution_eligibility(monkeypatch, tmp_path):
    ...
    with pytest.raises(OutcomesStageError) as exc:
        run_outcomes_stage(runtime, candidate_file=candidate, trace_file=trace, cost_config_file=cost)
    assert "execution_not_after_signal" in str(exc.value)
...
def test_outcomes_requires_bid_and_ask(monkeypatch, tmp_path):
    ...
    with pytest.raises(OutcomesStageError) as exc:
        run_outcomes_stage(runtime, candidate_file=candidate, trace_file=trace, cost_config_file=cost)
    assert "trace_missing_fields" in str(exc.value)
...
def test_outcomes_blocks_when_no_complete_trace(monkeypatch, tmp_path):
    ...
    assert artifact.name == "outcomes.stage.json"
    assert artifact.is_file()
    assert result.output_hashes[str(artifact)] == sha256_file(artifact)
    assert payload["decision"] == "OUTCOME_EVIDENCE_INSUFFICIENT"
    assert payload["summary"]["complete_count"] == 0
    assert payload["summary"]["insufficient_trace_count"] == 1
...
def test_outcomes_rejects_extra_undeclared_input(monkeypatch, tmp_path):
    ...
    with pytest.raises(OutcomesStageError) as exc:
        run_outcomes_stage(runtime, candidate_file=candidate, trace_file=trace, cost_config_file=cost)
    assert "requires_truth_manifest" in str(exc.value)
