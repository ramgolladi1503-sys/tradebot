...
def _sample_trade():
    return Trade(
        ...
        expiry_date=now_ist().date().isoformat(),
        tradingsymbol=f"NIFTY{now_ist().date().strftime('%y%b').upper()}25000CE",
        instrument_id=f"NIFTY{now_ist().date().strftime('%y%b').upper()}25000CE",
        option_type="CE",
        right="CE",
        side="BUY",
        entry_price=100.0,
