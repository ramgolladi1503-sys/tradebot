...
def test_morning_trade_uses_previous_state_not_same_day_1500_state():
    baseline = pd.DataFrame(
        {
            "entry_timestamp": [
                pd.Timestamp("2026-08-24 09:20", tz="Asia/Kolkata"),
                pd.Timestamp("2026-08-24 15:10", tz="Asia/Kolkata"),
            ],
            "net_pnl": [1.0, -1.0],
        }
    )
    states = pd.DataFrame(
        {
            "state_timestamp": [
                pd.Timestamp("2026-08-21 15:00", tz="Asia/Kolkata"),
                pd.Timestamp("2026-08-24 15:00", tz="Asia/Kolkata"),
            ],
            "vrp_zscore": [-1.0, 1.0],
    ...
    joined, metrics = evaluate_buyonly_gate.evaluate(
        baseline,
        states,
        entry_timestamp_column="entry_timestamp",
        pnl_column="net_pnl",
        assume_ist_baseline=False,
        assume_ist_states=False,
    )
...
def test_unavailable_state_is_not_silently_counted_as_rejected_trade():
    baseline = pd.DataFrame(
        {
            "entry_timestamp": [
                pd.Timestamp("2026-08-20 09:20", tz="Asia/Kolkata"),
                pd.Timestamp("2026-08-24 09:20", tz="Asia/Kolkata"),
            ],
            "net_pnl": [-5.0, 2.0],
        }
    ...
    _, metrics = evaluate_buyonly_gate.evaluate(
        baseline,
        states,
        entry_timestamp_column="entry_timestamp",
        pnl_column="net_pnl",
