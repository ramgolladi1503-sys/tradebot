...
def write_underlying(root: Path, date: str, filename: str, family: str):
    ...
    if path.suffix == ".csv":
        path.write_text(
            ...
            f"{date[:4]}-{date[4:6]}-{date[6:8]}T09:15:00,{family},100,101,99,100.5,10,false\n",
            encoding="utf-8",
        )
    else:
...
def test_inventory_reports_actual_per_family_session_coverage(tmp_path):
    ...
    write_underlying(root, "20240604", "NSE_INDEX|Nifty Bank_20240604.csv", "BANKNIFTY")
...
def test_historical_cache_gate_blocks_insufficient_sessions(tmp_path):
    root = tmp_path / "tradebot_historical_data"
    file_path = write_underlying(root, "20240604", "NSE_INDEX|Nifty Bank_20240604.csv", "BANKNIFTY")
...
def test_historical_cache_builds_only_selected_underlying_family(tmp_path):
    root = tmp_path / "tradebot_historical_data"
    for date in ("20240603", "20240604"):
        write_underlying(root, date, f"NSE_INDEX|Nifty Bank_{date}.csv", "BANKNIFTY")
        write_underlying(root, date, f"BSE_INDEX|SENSEX_{date}.csv", "SENSEX")
