...
def _sealed_root(tmp_path: Path, *, graph: bool = True, order_authority: bool = False) -> Path:
    root = tmp_path / "evidence"
    _write_json(root / "live" / "pr763_live_acceptance.json", _proof(graph=graph, order_authority=order_authority))
    _write_json(
...
def test_missing_live_semantic_proof_remains_pending_not_fabricated_pass(tmp_path: Path):
    ...
    live_gate = next(gate for gate in report["gates"] if gate["gate_id"] == "PR763_LIVE_SEMANTICS")
...
def test_post_seal_mutation_fails_hash_authority(tmp_path: Path):
    ...
    assert any("artifact_sha_mismatch" in error or "artifact_size_mismatch" in error for error in gate.evidence["errors"])


def test_unsafe_fallback_cannot_appear_in_executable_bucket(tmp_path: Path):
    root = _sealed_root(tmp_path)
    snapshot = _authority_snapshot(tmp_path / "authority.json", unsafe_executable=True)
    report = certify_pr763_session(
        evidence_root=root,
        authority_snapshot_paths=[snapshot],
        generated_at="2026-08-03T00:00:00+00:00",
    ...
    gate = next(gate for gate in report["gates"] if gate["gate_id"] == "RUNTIME_AUTHORITY_SNAPSHOTS")
    assert any("unsafe_quote_in_executable" in error for error in gate["evidence"]["errors"])
...
def test_any_order_authority_in_session_evidence_fails_closed(tmp_path: Path):
    ...
    assert report["verdict"] == FAILED_VERDICT
    gate = next(gate for gate in report["gates"] if gate["gate_id"] == "READ_ONLY_NO_ORDER_AUTHORITY")
    assert any("order_authority=true" in violation for violation in gate["evidence"]["violations"])


def test_empty_but_explicit_authority_snapshot_is_valid_for_zero_candidate_session(tmp_path: Path):
    root = _sealed_root(tmp_path)
    snapshot = tmp_path / "authority-empty.json"
    _write_json(snapshot, {"authority_snapshot": True, "top_executable": [], "top_advisory": [], "blocked_debug": []})
    report = certify_pr763_session(
        evidence_root=root,
        authority_snapshot_paths=[snapshot],
        generated_at="2026-08-03T00:00:00+00:00",
    ...
    assert report["verdict"] == PASS_VERDICT
    authority_gate = next(gate for gate in report["gates"] if gate["gate_id"] == "RUNTIME_AUTHORITY_SNAPSHOTS")
