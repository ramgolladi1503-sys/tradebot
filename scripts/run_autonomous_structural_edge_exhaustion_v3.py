...
def combined_discovery(a: dict, b: dict, c: dict, clock_norms: dict) -> dict:
    families = []
    for record in a.get("families", []):
        families.append({**record, "batch": "A"})
    for record in b.get("families", []):
        families.append({**record, "batch": "B"})
    for record in c.get("families", []):
        families.append({**record, "batch": "C"})
    catalog = {
        "schema_version": 3,
        "campaign": CAMPAIGN,
        "principal_verdict": "AUTONOMOUS_THREE_BATCH_OUTCOME_BLIND_FAMILY_DISCOVERY_FROZEN",
        "batch_count": 3,
        ...
        "policy": {
            ...
            "batch_a_b_failed_families_reopened": False,
            "batch_c_definitions_use_absent_information_primitives": True,
...
def main() -> int:
    ...
    base_cross = build_cross_sectional_frame(raw, index_rows, universe["selected_symbols"], development_sessions)
    ...
    stage0 = {
        "principal_verdict": "PINNED_CONSTITUENT_INDEX_SOURCE_AUTHORIZED_FOR_THREE_BATCH_OUTCOME_BLIND_DISCOVERY",
        **source_info,
    ...
    stage1 = {
        "principal_verdict": "GLOBAL_CHRONOLOGICAL_SPLIT_FROZEN_FOR_THREE_BATCH_CAMPAIGN",
        "splits": splits,
    ...
    stage3 = combined_discovery(a_discovery, b_discovery, c_discovery, clock_norms)
    stage4 = hypothesis_catalog(stage3)
    models = {**a_models, **b_models, **c_models}
    assignments = {**a_assignments, **b_assignments, **c_assignments}
    ...
    stage6 = structural_screen_v3(stage5)
    ...
    if pre_final_candidates:
        unopened_base = build_cross_sectional_frame(raw, index_rows, universe["selected_symbols"], splits["unopened"])
        ...
        for family in needed:
            if family in a_models:
                ...
                unopened_assignments[family] = assign_family(unopened_b, b_models[family])
            elif family in c_models:
                unopened_assignments[family] = assign_family(unopened_c, c_models[family])
            else:
    ...
    stage11 = exhaustion_ledger_v3(stage3, stage6, stage7, stage8, stage9)

    survivors = list(stage9.get("survivor_hypothesis_ids", []))
    principal = (
        "STATISTICAL_STRUCTURAL_EDGE_SURVIVOR_FOUND_AFTER_THREE_BATCH_CAMPAIGN_MEMBERSHIP_AUTHORITY_REQUIRED"
        if survivors
        else "NO_STRUCTURAL_EDGE_SURVIVED_THREE_BATCH_CONSTITUENT_INFORMATION_EXHAUSTION"
    )
    final = {
        ...
        "campaign_batch_count": 3,
        "campaign_family_count": len(ALL_FAMILIES_V3),
        "campaign_global_q_threshold": CAMPAIGN_GLOBAL_Q_V3,
    ...
    for name, payload in stages.items():
        stable_write(output / f"{name}.json", payload)
    (output / "REPORT.md").write_text(build_report_v3(stages), encoding="utf-8")
