...
class RealPaperMonitor:
    ...
    def save_health(self):
        ...
        health_data = {
            ...
            "closed_signal_count": len([s for s in self.paper_log if s['status'] != 'OPEN']),
    ...
    def build_candles(self, live_price, ts):
        ...
        c_1m = Candle("NIFTY", m_start, live_price-2, live_price+2, live_price-5, live_price, 1000, live_price)
        ...
        if len(self.df_1m_buffer) > 100: self.df_1m_buffer.pop(0)

        # 15m Candle logic
        m15_start = ts.replace(minute=(ts.minute//15)*15, second=0, microsecond=0)
        c_15m = Candle("NIFTY", m15_start, live_price-10, live_price+20, live_price-20, live_price, 15000, live_price)
        
        if not self.df_15m_buffer or self.df_15m_buffer[-1].timestamp != c_15m.timestamp:
            self.df_15m_buffer.append(c_15m)
        else:
            self.df_15m_buffer[-1] = c_15m

        if len(self.df_15m_buffer) > 50: self.df_15m_buffer.pop(0)
        return c_1m, c_15m
    ...
    def evaluate_signals(self, c_1m, c_15m, live_price, now_ts):
        ...
        res = self.strat.evaluate(df_15m, df_1m, c_15m, c_1m, regime, ablation="BASELINE")
        if isinstance(res, Signal):
            ...
            bid = quotes[opt_symbol].get("depth", {}).get("buy", [{'price': 150}])[0]['price']
            ask = quotes[opt_symbol].get("depth", {}).get("sell", [{'price': 151}])[0]['price']
            spread = ask - bid
    ...
    def track_open_signals(self, live_price):
        for sig in self.active_signals:
            ...
            if is_long:
                ...
                if live_price >= tg: self.close_signal(sig, live_price, "TARGET")
                elif live_price <= sl: self.close_signal(sig, live_price, "STOP")
            else:
    ...
    def close_signal(self, sig, live_price, reason):
        ...
        sig["status"] = reason
        move = (live_price - sig["nifty_spot"]) if sig["is_long"] else (sig["nifty_spot"] - live_price)
        sig["realized_R"] = round((move * 0.50 - sig["spread"]) / (sig["risk"] * 0.50), 2)
        ...
        try:
            while True:
                ...
                if 9 <= now.hour <= 15:
                    ...
                    if self.last_tick_time and (time.time() - self.last_tick_time) > FEED_STALE_THRESHOLD_SEC:
...
if __name__ == "__main__":
    ...
    monitor.active_signals.append({
        "signal_id": monitor.generate_signal_id(now.isoformat(), "VOL_EXPANSION", 23100),
