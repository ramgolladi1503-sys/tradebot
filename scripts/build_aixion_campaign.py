...
def main() -> int:
    ...
    summary = summarize_campaign(sessions, minimum_valid_sessions=args.minimum_valid_sessions, minimum_expiry_sessions=args.minimum_expiry_sessions, minimum_non_expiry_sessions=args.minimum_non_expiry_sessions, require_all_diagnosis_ready=args.require_all_diagnosis_ready, require_live_shadow_for_all_valid=args.require_live_shadow_for_all_valid)
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(summary.to_record(), indent=2, sort_keys=True, allow_nan=False), encoding="utf-8")
