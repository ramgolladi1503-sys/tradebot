...
STRONG_GRADE_REQUIREMENTS = {
    ...
    "Grade A": (
        "independent_attack_ref", "calibration_ref",
        "scientific_certification_ref", "economic_certification_ref",
    ),
    "Grade A+": (
        "independent_attack_ref", "calibration_ref",
        "live_forward_evidence_ref", "monitoring_ref",
    ),
}
...
def missing_required(inp, *fields):
    return [f for f in fields if f not in inp or inp[f] is None or inp[f] == "" or inp[f] == []]
...
def promotion(inp):
    ...
    for gate in required_gates:
        ...
        if bound_ref is None or authoritative_ref is None:
            return invalid_schema(can_promote=False)
        if bound_ref not in new_refs or authoritative_ref != bound_ref:
            return no_new_evidence()
...
def load_cases():
    ...
    if len(ids) != len(set(ids)):
        ...
        raise SystemExit(2)
    if not REQUIRED_V7_REPLACEMENTS.issubset(set(ids)):
        print("FIXTURE_V7_REPLACEMENT_MISSING")
        raise SystemExit(2)
