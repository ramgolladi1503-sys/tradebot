...
No round identifier is hard-coded. Repository evidence determines the active
candidate/population. Completed dirty populations cause one bounded repair
generation; repaired heads are calibrated and freshly reviewed automatically.
Stale historical populations are preserved but never re-consumed as current.
"""
...
def repair_evidence_for_failed(auth:Path,failed:str)->dict|None:
...
def record_aggregate(auth:Path,aggregate:dict,kind:str,round_id:str,candidate:str):
 ...
 (auth/ap).parent.mkdir(parents=True,exist_ok=True);(auth/ap).write_text(json.dumps(aggregate,sort_keys=True,indent=2)+'\n',encoding='utf-8');changed=[ap];decision=str(aggregate.get('decision'));state=(auth/STATE).read_text(encoding='utf-8');repair=None
 if decision not in PASS:
  findings=blocking_findings(aggregate,kind);repair={'schema_version':'mros-repair-contract-v2','sprint':'S003','failed_head':candidate,'source_kind':kind,'source_round':round_id,'aggregate_decision':decision,'blocking_findings':findings,'invalid_artifacts':aggregate.get('invalid',[]),'root_cause_instruction':'Cluster related findings and repair common causes once; do not weaken gates.','repair_scope':{'allowed':['scripts/mros/','tests/mros/','research/review_board/','research/audit_board/'],'forbidden':['research/program/','runtime/strategy/risk/execution/broker code','weaken fixtures or acceptance criteria','begin M9','create runtime authority']},'runtime_authority':'NONE','m9_status':'NOT_STARTED'};(auth/cp).write_text(json.dumps(repair,sort_keys=True,indent=2)+'\n',encoding='utf-8');changed.append(cp);state=set_top(state,'active_sprint_status',f'BOARD_AUTONOMOUS_{round_id}_{kind.upper()}_REPAIR_REQUIRED')
...
def calibration_status(q:Path,candidate:str):
 ...
 any_pending=False;any_infra_fail=False
 for rp,r in reversed(reqs):
  rec=q/ROOT/'receipts'/rp.name;out=q/str(r.get('output_path',''))
  if not rec.is_file():any_pending=True;continue
  try:rr=read_json(rec);job=rr.get('job',{})
  except Exception:any_infra_fail=True;continue
  if out.is_file():
  ...
  if job.get('state')!='SUCCEEDED' or job.get('exit_code')!=0:any_infra_fail=True
 if any_pending:return 'PENDING',None
 return ('INFRA_FAILED',None) if any_infra_fail else ('VALIDATION_FAILED',None)
def next_calibration_role(q:Path):
 nums=[]
 for p,r in calibration_requests(q,'') if False else []:pass
 d=q/ROOT/'requests'
 if d.is_dir():
...
def queue_review(q:Path,candidate:str):
 for n,_,m in manifests(q,'reviewer'):
  if m.get('candidate_head')==candidate:return f'R{n:03d}'
 n=next_review_round(q);round_id=f'R{n:03d}';members=[];paths=[]
...
def process_review(auth:Path,q:Path,state_root:Path,row):
 ...
 mod=load_mod(auth,'aggregate_reviews');aggregate=mod.aggregate_payloads(payloads,candidate_head=candidate,receipts=receipts,manifest=m);aggregate.update({'review_round':round_id,'population_manifest':str(mp.relative_to(q)),'runtime_authority':'NONE'});decision,cp=record_aggregate(auth,aggregate,'review',round_id,candidate);sync(auth,q)
 if decision not in PASS:
...
def process_audit(auth:Path,q:Path,state_root:Path,row):
 n,mp,m=row;audit_round=f'A{n:03d}';review_round=str(m.get('review_round') or '');candidate=m.get('candidate_head')
...
def main()->int:
 ...
 try:
  ...
  # After an autonomous repair the authority head is a new exact candidate not yet
  # represented by a review manifest. Calibrate/review it before touching history.
  if latest and repair_evidence_for_failed(auth,str(latest[2].get('candidate_head',''))):
   ...
   if not any(m.get('candidate_head')==cur for _,_,m in reviews):
    ...
    rr=queue_review(q,cur);print(json.dumps({'action':'REVIEW_QUEUED','candidate':cur,'round':rr,'native_ref':nref},sort_keys=True));return 0
  if latest:
   ...
   if (auth/rap).is_file() and read_json(auth/rap).get('decision') in PASS:
    ...
    if audit:
     result=process_audit(auth,q,root,audit);print(json.dumps(result,sort_keys=True));return 3 if result['action']=='WAIT_AUDIT' else 0
    queue_audit(q,auth,candidate,round_id,read_json(auth/rap));print(json.dumps({'action':'AUDIT_QUEUED','candidate':candidate,'round':round_id},sort_keys=True));return 0
