...
def aggregate_payloads(payloads, *, candidate_head, review_round):
 audits=[];invalid=[];seen_roles=set();seen_artifacts=set()
 for name,d in payloads:
  ...
  if errs:
   invalid.append({'file':str(name),'audit':d,'errors':errs})
  else:
   audits.append(d);seen_roles.add(role);seen_artifacts.add(artifact)
