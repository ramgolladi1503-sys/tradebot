...
def aggregate_payloads(payloads,*,candidate_head,review_round,receipts,manifest):
 manifest_errors=validate_population_manifest(manifest,candidate_head=candidate_head,job_type='auditor')
 artifacts=[d for _,d in payloads if isinstance(d,dict)];recon=reconcile_population(manifest,artifacts) if not manifest_errors else {'expected':0,'submitted':len(artifacts),'omitted':[],'extra':[],'expected_members':{}}
...
def main():
 ...
 out=aggregate_payloads(payloads,candidate_head=a.candidate_head,review_round=a.review_round,receipts=load_receipts(Path(a.receipt_dir)),manifest=manifest)
