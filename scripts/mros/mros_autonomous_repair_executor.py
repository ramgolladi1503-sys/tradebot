...
def execute(*,repo:Path,state_root:Path,contract_path:Path,generation:int)->dict:
 ...
 git(repo,'fetch','origin',AUTH,timeout=300);base=git(repo,'rev-parse',f'origin/{AUTH}').stdout.strip();contract=json.loads(contract_path.read_text(encoding='utf-8'));failed=contract.get('failed_head')
 ...
 try:
  ...
  agent=run(wt,'codex','exec','--ephemeral','--sandbox','workspace-write','--color','never',prompt(contract,generation),timeout=int(os.environ.get('MROS_CODEX_REPAIR_TIMEOUT_SECONDS','3600')),check=False,env=env)
  ...
  git(wt,'config','user.name','MROS Autonomous Repair');git(wt,'config','user.email','mros-autonomous@local.invalid');git(wt,'commit','-m',f"mros({contract.get('sprint','S003')}): autonomous repair generation {generation} [skip ci]");code_commit=git(wt,'rev-parse','HEAD').stdout.strip()
  evidence_rel=f"research/evidence/sprints/{contract.get('sprint','S003')}/AUTONOMOUS_REPAIR_G{generation}_{code_commit[:8]}.json";evidence=wt/evidence_rel;evidence.parent.mkdir(parents=True,exist_ok=True)
  evidence.write_text(json.dumps({'schema_version':'mros-autonomous-repair-execution-v1','sprint':contract.get('sprint'),'generation':generation,'failed_head':failed,'repair_base':base,'code_commit':code_commit,'changed_paths':paths,'agent_exit_code':agent.returncode,'runtime_authority':'NONE','broker_actions':'NONE','m9_status':'NOT_STARTED','recorded_at':time.time()},sort_keys=True,indent=2)+'\n',encoding='utf-8');git(wt,'add','--',evidence_rel);git(wt,'commit','-m',f"mros({contract.get('sprint','S003')}): seal autonomous repair generation {generation} evidence [skip ci]");candidate=git(wt,'rev-parse','HEAD').stdout.strip()
