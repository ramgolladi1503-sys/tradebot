...
This process removes ChatGPT from the polling/scheduling loop. It observes the
authority and mailbox branches, keeps a durable local state/health record, runs
only deterministic repository automation hooks, and uses a non-certifying
control lane for controller/builder/repair work. Certifying reviewer/auditor
outputs remain exclusively on the independent Mac mailbox bridge.
"""
...
@dataclass
class Health:
    supervisor_status:str="STARTING"; phase:str="UNKNOWN"; authority_head:str=""; queue_head:str=""; milestone:str=""; work_package:str=""; sprint:str=""
    pending_requests:int=0; completed_receipts:int=0; failed_receipts:int=0; worker_alive:bool=False; next_action:str="DISCOVER"; last_error:str|None=None
    runtime_authority:str="NONE"; m9_started:bool=False; updated_at:float=0.0
...
def parse_program_state(text:str)->dict[str,str]:
    keys=("active_milestone","active_work_package","active_sprint","program_status","active_sprint_status")
    out={}
    for k in keys:
...
def read_at_ref(repo:Path,git_ref:str,path:str)->str:
    return git(repo,"show",f"{git_ref}:{path}").stdout
...
def derive_phase(state:dict[str,str],requests:list[str],receipts:dict[str,dict[str,Any]])->tuple[str,str]:
    ...
    names={Path(x).name for x in requests};done=set(receipts)
    pending=names-done
...
def worker_heartbeat(state_root:Path,stale_seconds:int)->bool:
    events=state_root/"events.jsonl"
    if not events.exists():return False
    try:return time.time()-events.stat().st_mtime<=stale_seconds
...
def run_step(repo:Path,step_script:Path,health_path:Path)->int:
    ...
    p=subprocess.run([sys.executable,str(step_script),"--repo",str(repo),"--health",str(health_path),"--once"],cwd=repo,timeout=900,check=False)
    return p.returncode
    ...
    try:
        while True:
            try:
                ...
                req,rec=queue_inventory(repo);names={Path(x).name for x in req};h.pending_requests=len(names-set(rec));h.completed_receipts,h.failed_receipts=receipt_stats(rec);h.worker_alive=worker_heartbeat(root,300);h.phase,h.next_action=derive_phase(state,req,rec)
