...
def main()->int:
 ...
 for m in manifest.get('members',[]):
  ...
  d=read_json(out);r=read_json(rec);payloads.append((m['output_path'],d));job=r.get('job') if isinstance(r,dict) else None
 ...
 aggregate['audit_round']='A001';aggregate['population_manifest']=MANIFEST.as_posix();aggregate['review_aggregate']=REVIEW_AGG.as_posix();aggregate['runtime_authority']='NONE';aggregate['m9_started']=False
 ...
 if decision in PASS:
  state=replace_scalar(state,'active_sprint_status','BOARD_BOOTSTRAP_AUTHORIZATION_PENDING')
  state=replace_indented(state,'bootstrap_independent_audit_status',str(decision))
 else:
  findings=[]
