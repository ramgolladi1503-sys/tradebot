...
def aggregate_payloads(payloads, *, candidate_head):
 reviews=[];invalid=[];seen_roles=set();seen_artifacts=set()
 for name,d in payloads:
  errs=validate(d,candidate_head)
  role=d.get('role') if isinstance(d,dict) else None
  artifact=d.get('artifact_id') if isinstance(d,dict) else None
  ...
  if errs:
   invalid.append({'file':str(name),'review':d,'errors':errs})
  else:
   reviews.append(d);seen_roles.add(role);seen_artifacts.add(artifact)
...
def main():
 ...
 out=aggregate_payloads(payloads,candidate_head=a.candidate_head);out['native_validation']='PASS'
