...
def aggregate_payloads(payloads,*,candidate_head,receipts,manifest):
 ...
 for name,d in payloads:
  errs=validate(d,candidate_head);member=recon['expected_members'].get(d.get('output_path')) if isinstance(d,dict) else None
...
def main():
 ...
 payloads=[(str(f),json.loads(f.read_text())) for f in sorted(Path(a.review_dir).glob('reviewer-*.json'))];manifest=json.loads(Path(a.population_manifest).read_text())
