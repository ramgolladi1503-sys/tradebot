...
def main():
 ...
 if any(k not in n for k in required) or n.get('head')!=a.candidate_head or n.get('failed')!=0 or n.get('exit_code')!=0 or n.get('passed')!=n.get('checks'):
 ...
 out={'candidate_head':a.candidate_head,'native_validation':'PASS','minimum_valid_reviews':MIN_VALID,'valid_reviews':len(reviews),'invalid_reviews':len(invalid),'pass_or_minor':good,'critical':critical,'major':major,'minor':minor,'unknown':unknown,'decision':decision,'reviews':reviews,'invalid':invalid}
