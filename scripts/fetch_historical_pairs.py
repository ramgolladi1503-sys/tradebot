...
def main():
    ...
    print(
        f"Resolved Tokens: NIFTY 50 -> {symbol_to_token['NIFTY 50']}, NIFTY BANK -> {symbol_to_token['NIFTY BANK']}"
    )
    ...
    print(
        f"Fetching 1-minute historical data from {from_date.strftime('%Y-%m-%d')} to {to_date.strftime('%Y-%m-%d')}..."
    )
    ...
    try:
        ...
        print(
            f"ERROR: Failed to fetch historical data. Make sure you have the historical data add-on. Error: {e}"
        )
        sys.exit(1)
    ...
    common_ts = sorted(
        list(set(nifty_map.keys()).intersection(set(banknifty_map.keys())))
    )
