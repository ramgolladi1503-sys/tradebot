...
from core.intelligence.calibration.factors import (
    Factor,
    FactorOrigin,
    CalibrationStatus,
)
...
def run_e2e():
    ...
    with store._get_connection() as conn:
        conn.execute(
            "INSERT INTO intelligence_sources (source_id, url) VALUES (?, ?)",
    ...
    run_id = store.insert_fetch_run(
        "SEBI",
        "https://sebi.gov.in/test",
        time.time(),
        "success",
        fetch_latency,
        200,
    ...
    print(
        f"2. **Source & Fetch**: Simulated HTTP fetch. Registered in DB run_id: `{run_id}`. Emitted `fetch_succeeded` telemetry."
    )

    # 3. Extraction
    extractor = SEBIExtractor("sebi.gov.in")
    extracted = extractor.safe_extract(
        payload["raw_content"], "https://sebi.gov.in/test"
    )
    telemetry.emit_extraction_event(
        "extraction_succeeded", "SEBI", "success", extracted["document_hash"]
    )
    print(
        f"3. **Extraction**: SEBIExtractor parsed title: `{extracted['title']}` with parser version `{extracted['parser_version']}`."
    )
    ...
    with store._get_connection() as conn:
        conn.execute(
            "INSERT INTO intelligence_documents (doc_id, run_id, content_hash, title, raw_content) VALUES (?, ?, ?, ?, ?)",
            (
                doc_id,
                run_id,
        ...
        conn.execute(
            "INSERT INTO intelligence_events (event_id, doc_id, advisory_only, calibration_status, evidence_pointer) VALUES (?, ?, 1, ?, ?)",
    ...
    telemetry.emit_calibration_event(
        "replay_calibration_insufficient", "SEBI", replay_res["calibration_status"]
    )
    print(
        f"5. **Replay Engine**: Evaluated event against historic `tick_store`. Correctly returned: `{replay_res['calibration_status']}`."
    )
    ...
    print(
        f"6. **Advisory Context**: ContextAdapter injected intelligence. Final candidate `execution_ok` status remains: `{mutated['execution_ok']}`. System absolutely isolated."
    )
