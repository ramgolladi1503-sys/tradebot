...
def run():
    ...
    for file in files:
        ...
        blocked = [
            c
            for c in candidates
            if c.get("decision", {}).get("status_raw")
            in ["blocked", "PLANNING", "advisory_only", "QUEUE_ONLY"]
            or c.get("decision", {}).get("permission_base") == "QUEUE_ONLY"
        ]
        ...
        for c in blocked[-5:]:
            ...
            print(
                f"ID: {adv_id} | Blockers: {decision.get('blockers', [])} | Entry: {entry}"
            )
