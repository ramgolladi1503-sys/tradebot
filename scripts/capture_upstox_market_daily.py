...
def fetch_bod_master():
    ...
    try:
        ...
        with gzip.open(out_path.with_suffix(".json.gz"), "rt", encoding="utf-8") as f:
            jdata = json.load(f)
        ...
        logger.info(f"Successfully refreshed BOD JSON. Saved {len(jdata)} instruments.")
        return True
...
def preflight_auth(token):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.error(f"Preflight auth failed. Status: {resp.status_code}, Response: {resp.text}")
            if "UDAPI1221" in resp.text:
                logger.error("=> UPSTOX IP WHITELIST ERROR: The API key does not allow this machine's IP. Please whitelist this VPS/machine IP in the Upstox Developer Console.")
            return False
    except Exception as e:
...
def _find_nearest_expiry(instruments, symbol_name):
    ...
    opts = [i for i in instruments if i.get("name") == symbol_name and i.get("instrument_type") in ["CE", "PE"] and i.get("expiry")]
...
def _find_futures(instruments, symbol_name):
    today = datetime.now().date()
    futs = [i for i in instruments if i.get("name") == symbol_name and i.get("instrument_type") == "FUT"]
    valid = []
    for fut in futs:
        try:
...
def resolve_instruments():
    ...
    nifty_spot = [i for i in instruments if i.get("trading_symbol") == "NIFTY 50" and i.get("instrument_type") == "INDEX"]
    if not nifty_spot:
    ...
    if not opts:
        ...
        return None
    for o in opts: resolved_keys.add(o["instrument_key"])
        
    futs = _find_futures(instruments, "NIFTY")
    for f in futs: resolved_keys.add(f["instrument_key"])
        
    # 2. NIFTY BANK spot + options + futures
    bank_spot = [i for i in instruments if i.get("trading_symbol") == "NIFTY BANK" and i.get("instrument_type") == "INDEX"]
    if not bank_spot:
    ...
    sensex_spot = [i for i in instruments if i.get("trading_symbol") == "SENSEX" and i.get("instrument_type") == "INDEX"]
    if not sensex_spot:
    ...
    # 4. India VIX
    vix_spot = [i for i in instruments if i.get("trading_symbol") == "INDIA VIX" and i.get("instrument_type") == "INDEX"]
    if not vix_spot:
...
class DataCollector:
    ...
    def on_market_update(self, message):
        ...
        try:
            # We must parse the message payload correctly. Upstox returns dictionary for V3
            for key, data in message.items():
                if isinstance(data, dict):
                    ...
                    rec = {
                        "ts": time.time(),
                        "instrument_key": key,
                        "ltp": float(data.get("ltpc", {}).get("ltp", 0.0)) if data.get("ltpc") else None,
                        "volume": int(data.get("ff", {}).get("market_ff", {}).get("vtt", 0)) if data.get("ff") else None,
                        "oi": float(data.get("ff", {}).get("market_ff", {}).get("oi", 0.0)) if data.get("ff") else None
                    }
                    ...
                    buy = depth.get("buy", [])
                    sell = depth.get("sell", [])
                    rec["bid_price"] = float(buy[0].get("price", 0.0)) if buy else None
                    rec["ask_price"] = float(sell[0].get("price", 0.0)) if sell else None
                    
                    # Greeks
                    option_greeks = data.get("option_greeks", {})
                    rec["delta"] = float(option_greeks.get("delta", 0.0)) if option_greeks else None
                    rec["theta"] = float(option_greeks.get("theta", 0.0)) if option_greeks else None
                    rec["gamma"] = float(option_greeks.get("gamma", 0.0)) if option_greeks else None
                    rec["vega"] = float(option_greeks.get("vega", 0.0)) if option_greeks else None
                    rec["iv"] = float(option_greeks.get("iv", 0.0)) if option_greeks else None
    ...
    def on_close(self, code, reason):
        logger.warning(f"Upstox WebSocket closed (Code: {code}, Reason: {reason}).")
        self.reconnects += 1
...
def main():
    ...
    if not UPSTOX_AVAILABLE:
        logger.error("Upstox client not available (pip install upstox-python-sdk). Exiting.")
        sys.exit(1)
