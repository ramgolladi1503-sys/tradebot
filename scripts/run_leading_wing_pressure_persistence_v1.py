#!/usr/bin/env python3
"""Leading-wing informed-pressure persistence discovery.

A distinct continuation family: buy the option wing already leading only when
same-strike mirror decay and causal surface/OI/volume evidence confirm. Thresholds
are training-only inside four expanding WFA folds. Latest 25% holdout stays sealed
until an OOF survivor is frozen. Research only; no broker/paper/live action.
"""
...
OUT_REL = Path("runtime/research/leading_wing_pressure_persistence_v1")
RESEARCH_REL = Path("research/leading_wing_pressure_persistence_v1")
...
MECHANISMS = (
    "wing_lead_second_push",
    "opposing_wing_decay_persistence",
    "oi_volume_informed_pressure",
    "breadth_mass_migration",
    "accepted_two_bar_pressure",
    "low_dispersion_directional_lift",
    "volume_acceleration_continuation",
    "mirror_decay_breadth_acceleration",
    "near_expiry_informed_pressure",
    "late_session_informed_pressure",
)
...
def thresholds(training: pd.DataFrame) -> dict[str, float]:
    return {
        "ret60": q(training, "prior_5m_return_pct", 0.60),
        "ret70": q(training, "prior_5m_return_pct", 0.70),
        ...
        "bacc70": q(training, "breadth_acceleration", 0.70),
...
def masks(frame: pd.DataFrame, cut: dict[str, float]) -> dict[str, pd.Series]:
    ...
    lead = (
        ...
        & (ret > 0)
        & (asym >= cut["asym60"])
        & (breadth >= max(0.50, cut["breadth60"]))
    )
    second = (prev > 0) & (ret > prev) & (acc >= cut["acc70"])
    mirror_decay = (mirror <= 0) & (macc <= 0)

    result = {
        "wing_lead_second_push": lead & second & mirror_decay & (vol >= cut["vol60"]),
        "opposing_wing_decay_persistence": (
            lead & (prev > 0) & mirror_decay & (asym >= cut["asym70"]) & (vol >= cut["vol60"])
        ),
        "oi_volume_informed_pressure": (
            lead & (acc >= cut["acc60"]) & (vol >= cut["vol70"]) & (oi >= cut["oi70"]) & (mirror <= 0)
        ),
        "breadth_mass_migration": (
            (ret >= cut["ret70"])
            & (breadth >= max(0.60, cut["breadth70"]))
            & (bdelta >= cut["bdelta60"])
            & (mass >= cut["mass70"])
            & (vol >= cut["vol60"])
        ),
        "accepted_two_bar_pressure": (
            lead & (prev > 0) & frame["bar_acceptance"].fillna(False) & mirror_decay & (vol >= cut["vol60"])
        ),
        "low_dispersion_directional_lift": (
            (smedian > 0)
            & (sacc > 0)
            & (dispersion <= cut["disp35"])
            & (breadth >= max(0.70, cut["breadth70"]))
            & (ret >= cut["ret60"])
            & mirror_decay
        ),
        "volume_acceleration_continuation": (
            lead & (acc >= cut["acc70"]) & (vol >= cut["vol70"]) & (vacc >= cut["vacc75"]) & (mirror <= 0)
        ),
        "mirror_decay_breadth_acceleration": (
            (ret >= cut["ret60"])
            & mirror_decay
        ...
        "late_session_informed_pressure": (
            lead
            & frame["minute_of_day"].between(750, 870, inclusive="both")
            & (acc >= cut["acc70"])
            & (vol >= cut["vol70"])
            & mirror_decay
            & (bacc >= cut["bacc60"])
...
def eligible(frame: pd.DataFrame) -> pd.Series:
    return (
        ...
        & frame["mirror_acceleration"].notna()
    )
...
def select(frame: pd.DataFrame, mask: pd.Series, mechanism: str, sessions: list[str]) -> pd.DataFrame:
    ...
    candidates["score"] = (
        ...
        + candidates["option_asymmetry"].fillna(0)
...
def main() -> int:
    ...
    contract = {
        "schema_version": "leading_wing_pressure_persistence_v1",
        "mechanism_hypothesis": "leading_option_wing_informed_pressure_with_mirror_decay_and_surface_confirmation",
    ...
    verdict = (
        "STRUCTURAL_EDGE_FOUND_LEADING_WING_PRESSURE_PERSISTENCE_CANDLE_PROXY"
        if validated
        else ("NO_OOF_SURVIVOR_IN_LEADING_WING_PRESSURE_FAMILY" if not names else "OOF_SURVIVORS_FAILED_HOLDOUT_OR_CONTROLS")
    )
    final = {
    ...
    (research / "RESULT.md").write_text(
        "# Leading-Wing Pressure Persistence V1\n\n"
