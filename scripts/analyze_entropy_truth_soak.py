...
def is_top_opportunity(data):
    bucket = get_first(
        data, ["display_bucket", "bucket", "ranking_bucket", "ui_bucket"]
    )
    if not bucket:
        return False
...
def analyze_candidate(data):
    ...
    # Check fields
    raw_entropy = get_first(
        data, ["market_entropy_raw", "regime_entropy", "entropy", "raw_entropy"]
    )
    norm_entropy = get_first(
        data,
        [
    ...
    is_uncertain = get_first(
        data, ["market_regime_uncertain", "is_uncertain", "entropy_too_high"], False
    )
...
def analyze():
    ...
    if files["candidate"].exists():
        for line in files["candidate"].read_text().splitlines():
            ...
            bucket = normalize_display_bucket(
                get_first(
                    data, ["display_bucket", "bucket", "ranking_bucket", "ui_bucket"]
                )
            )
