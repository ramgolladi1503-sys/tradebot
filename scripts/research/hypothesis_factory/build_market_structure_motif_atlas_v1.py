...
def near(a,b,tol):return abs(bps(a,b))<=tol
...
def zone_motifs(rows,zones,tol,lookahead_bars=12):
 # descriptive only: motif becomes observable at retest/hold/fail event; no profitability computed
 ...
 for z in zones:
  ...
  for i in range(first_i,len(rows)):
   r=rows[i]
   d=bps(z['center'],r['close'])
   if (z['side']=='RESISTANCE' and d>tol) or (z['side']=='SUPPORT' and d<-tol):
    broken=i;break
  ...
  for j in range(broken+1,min(len(rows),broken+1+lookahead_bars)):
   ...
   d=abs(bps(z['center'],rows[j]['close']))
   if d<=tol:
    held=False
...
def context_motifs(rows,bar_fn,zones,tol,lookback):
 ...
 for i,r in enumerate(rows):
  d=bar_fn(rows,i,lookback);ps=set(d['primitives'])
  if not ps:continue
  for z in zones:
   if r['timestamp']<z['first_confirmation_timestamp']:continue
   if abs(bps(z['center'],r['close']))>tol:continue
   if 'RANGE_COMPRESSION' in ps:
   ...
   if z['side']=='RESISTANCE' and 'UPPER_WICK_REJECTION' in ps:
    out.append({'motif':'WICK_REJECTION_AT_RESISTANCE','confirmation_timestamp':r['timestamp'],'session':r['session'],'zone_id':z['zone_id'],'zone_center':z['center']})
   if z['side']=='SUPPORT' and 'LOWER_WICK_REJECTION' in ps:
    out.append({'motif':'WICK_REJECTION_AT_SUPPORT','confirmation_timestamp':r['timestamp'],'session':r['session'],'zone_id':z['zone_id'],'zone_center':z['center']})
 return out
...
def main(argv=None):
 ...
 try:
  ...
  motifs.sort(key=lambda x:(x['confirmation_timestamp'],x['motif']))
