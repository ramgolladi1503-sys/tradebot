...
RUNNER_ID = "OPENING_SESSION_MICROSTRUCTURE_DEVELOPMENT_OUTCOME_V1"
SEARCH_FAMILY_ID = "OPENING_SESSION_MICROSTRUCTURE_PROXY_FAMILY_V1"
...
def main():
    ...
    parser.add_argument("--candidates-input", default="research/evidence/behavior_discovery_engine_v2/NIFTY_opening_session_microstructure_proxy_candidates_v1.jsonl")
    parser.add_argument("--development-output", default="research/evidence/behavior_discovery_engine_v2/NIFTY_opening_session_microstructure_proxy_development_v1.json")
    ...
    if not dataset_path.exists() or not episodes_path.exists() or not candidates_path.exists():
        print(f"BLOCKED: Missing required dataset/episodes/candidates path")
        sys.exit(1)
    ...
    from build_opening_session_microstructure_proxy_candidates_v1 import get_opening_bucket
    ...
    for episode in episodes:
        ...
        op_bucket = get_opening_bucket(episode.get("end_timestamp"))
        
        for cand in candidates:
            if cand["opening_bucket"] != op_bucket: continue
    ...
    for cand in candidates:
        ...
        summary = {
            "candidate_id": cid,
            "opening_bucket": cand["opening_bucket"],
    ...
    if len(supported) > 2:
        ...
        status = "NO_DEVELOPMENT_SUPPORTED_OPENING_SESSION_MICROSTRUCTURE_CANDIDATE"
