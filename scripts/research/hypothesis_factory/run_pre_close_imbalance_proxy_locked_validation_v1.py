...
RUNNER_ID = "PRE_CLOSE_IMBALANCE_LOCKED_VALIDATOR_V1"
SEARCH_FAMILY_ID = "PRE_CLOSE_IMBALANCE_PROXY_FAMILY_V1"
...
def main():
    ...
    candidates_path = bde2_dir / "NIFTY_pre_close_imbalance_proxy_candidates_v1.jsonl"
    dev_evidence_path = bde2_dir / "NIFTY_pre_close_imbalance_proxy_development_v1.json"
    output_path = bde2_dir / "NIFTY_pre_close_imbalance_proxy_locked_validation_v1.json"
    
    if not dataset_path.exists() or not dev_evidence_path.exists():
    ...
    supported_dev_ids = set(dev_ev.get("supported_candidates", []))
    all_candidates = load_jsonl(candidates_path)
    candidates = [c for c in all_candidates if c["candidate_id"] in supported_dev_ids]
    ...
    from build_pre_close_imbalance_proxy_candidates_v1 import get_preclose_bucket
    ...
    for episode in episodes:
        ...
        pc_bucket = get_preclose_bucket(episode.get("end_timestamp"))
        
        for cand in candidates:
            if cand["preclose_bucket"] != pc_bucket: continue
    ...
    for cand in candidates:
        ...
        summary = {
            "candidate_id": cid,
            "preclose_bucket": cand["preclose_bucket"],
    ...
    status = "LOCKED_VALIDATION_SUPPORTED" if supported else "LOCKED_VALIDATION_FAILED"
