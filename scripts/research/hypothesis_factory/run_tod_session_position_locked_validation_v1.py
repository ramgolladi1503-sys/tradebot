...
RUNNER_ID = "TOD_SESSION_POSITION_LOCKED_VALIDATOR_V1"
SEARCH_FAMILY_ID = "TIME_OF_DAY_SESSION_POSITION_FAMILY_V1"
OUTCOME_SCOPE = "locked_sessions_only"
...
def main():
    ...
    parser.add_argument("--candidates-input", default="research/evidence/behavior_discovery_engine_v2/NIFTY_tod_session_position_candidates_v1_narrowed.jsonl")
    parser.add_argument("--development-evidence", default="research/evidence/behavior_discovery_engine_v2/NIFTY_tod_session_position_narrowed_development_v1.json")
    parser.add_argument("--output", default="research/evidence/behavior_discovery_engine_v2/NIFTY_tod_session_position_locked_validation_v1.json")
    ...
    dev_evidence_path = root / args.development_evidence
    ...
    # We load ONLY the frozen narrowed supported candidates that passed development
    with dev_evidence_path.open() as f:
        dev_ev = json.load(f)
    
    supported_dev_candidate_ids = set(dev_ev.get("supported_candidates", []))
    all_candidates = load_jsonl(candidates_path)
    candidates = [c for c in all_candidates if c["candidate_id"] in supported_dev_candidate_ids]
    ...
    for episode in episodes:
        ...
        # KEY: WE ONLY COMPUTE ON LOCKED SESSIONS
        ...
        for cand in candidates:
            if cand["tod_bucket"] != tod: continue
    ...
    for cand in candidates:
        ...
        reasons = []
        if len(rows) < 20: reasons.append("MIN_LOCKED_MATCHES_FAIL")
        if distinct_sessions < 15: reasons.append("MIN_DISTINCT_LOCKED_SESSIONS_FAIL")
        ...
        verdict = "LOCKED_STRUCTURE_SUPPORTED" if not reasons else "LOCKED_STRUCTURE_REJECTED"
        if not reasons:
    ...
    if supported:
        status = "LOCKED_VALIDATION_SUPPORTED"
    else:
        status = "LOCKED_VALIDATION_FAILED"
        
    out = {
        "schema_version": 1,
        "runner_id": RUNNER_ID,
        "search_family_id": SEARCH_FAMILY_ID,
        "input_development_evidence_sha256": sha256_file(dev_evidence_path),
        "input_narrowed_candidates_sha256": sha256_file(candidates_path),
        "dataset_sha256": sha256_file(dataset_path),
        "development_candidates_frozen": True,
        "locked_sessions": len(locked_sessions),
        "locked_matches_skipped_or_computed_policy": "STRICT_ISOLATION_AND_EVALUATION",
        "locked_outcomes_accessed": True,
        "locked_outcomes_scope": OUTCOME_SCOPE,
        "edge_claimed": False,
        "candidate_summaries": candidate_summaries,
        "supported_candidate_count": len(supported),
        "supported_candidates": supported,
        "status": status,
        "next_action": "NO_LOCKED_SUPPORTED_TOD_SESSION_POSITION_CANDIDATE" if not supported else "ADVANCE_TO_WFA_AND_ROBUSTNESS"
    }
