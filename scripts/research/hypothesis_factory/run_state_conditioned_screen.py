...
REQUIRED_PRIOR_STATE = ("prior_ret_bps", "prior_range_bps", "prior_close")
...
def load(path: Path, instrument: str) -> list[dict[str, Any]]:
    ...
    rows.sort(key=lambda r: (sk(r), ts(r)))
    return rows


def session_stats(s: list[dict[str, Any]]) -> dict[str, float]:
    o = f(s[0], "open", f(s[0], "close"))
    c = f(s[-1], "close")
    hi = max(f(x, "high") for x in s)
    lo = min(f(x, "low") for x in s)
    mid = (hi + lo) / 2
    return {
        "ret_bps": ret_bps(o, c),
        "range_bps": 0.0 if mid <= 0 else (hi - lo) / mid * 10000.0,
        "close": c,
    }
...
def prior_state(ordered: list[tuple[str, list[dict[str, Any]]]]) -> dict[str, dict[str, float]]:
    ...
    for day, s in ordered:
        ...
        if prev is not None:
            out[day] = {
                "prior_ret_bps": prev["ret_bps"],
                "prior_range_bps": prev["range_bps"],
...
def complete_prior_state(state: dict[str, float] | None) -> bool:
    return bool(state) and all(k in state for k in REQUIRED_PRIOR_STATE)
...
def regime_ok(h: dict[str, Any], state: dict[str, float], median_prior_range: float) -> bool:
    if not complete_prior_state(state):
        return False
    r = state["prior_ret_bps"]
    vr = state["prior_range_bps"]
    reg = h["regime"]
    if reg == "prior_up":
        return r > 0
    if reg == "prior_down":
        return r < 0
    if reg == "prior_high_vol":
        return vr >= median_prior_range
    if reg == "prior_low_vol":
        return vr < median_prior_range
    return False
...
def signal(h: dict[str, Any], s: list[dict[str, Any]], i: int, state: dict[str, float]) -> bool:
    if not complete_prior_state(state):
        return False
    if not regime_ok(h, state, state.get("median_prior_range", 0.0)):
        return False
    d = h["direction"]
    fam = h["family"]
    c = f(s[i], "close")
    o = f(s[0], "open", f(s[0], "close"))
    pc = state["prior_close"]
    if c <= 0:
        return False
    ...
    if i < n:
        return False
    prev = s[i - n:i]
    ph = max(f(x, "high") for x in prev)
    pl = min(f(x, "low") for x in prev)
    if fam == "intraday_breakout":
        if d == 1:
            return c > ph and ret_bps(ph, c) >= h["threshold_bps"]
        return c < pl and ret_bps(c, pl) >= h["threshold_bps"]
    if fam == "intraday_reversion":
        hi = f(s[i], "high")
        lo = f(s[i], "low")
        if d == 1:
            return lo < pl and c > pl and ret_bps(lo, pl) >= h["threshold_bps"]
...
def evaluate(
    ...
    pnls = []
    by = defaultdict(list)
    prior_ranges = [v["prior_range_bps"] for v in state_map.values() if complete_prior_state(v)]
    med = statistics.median(prior_ranges) if prior_ranges else 0.0
    skipped_missing_prior_state = 0

    for day, s in ordered:
        base_state = state_map.get(day)
        if not complete_prior_state(base_state):
            skipped_missing_prior_state += 1
            continue
        st = dict(base_state)
        st["median_prior_range"] = med
        i = 1
        while i < len(s) - 1:
            if signal(h, s, i, st):
    ...
    abs_s = [abs(sum(v)) for v in by.values()]
    total = sum(abs_s)
    return {
        **h,
        "hypothesis_id": h["id"],
        "direction": "BUY_CE" if h["direction"] == 1 else "BUY_PE",
        "trades": n,
        "sessions_traded": len(by),
        "sessions_skipped_missing_prior_state": skipped_missing_prior_state,
        "win_rate": round(len(wins) / n, 4) if n else 0.0,
        "net_expectancy_bps": round(exp, 4),
        "profit_factor": "INF" if math.isinf(pf) else round(pf, 4),
        "max_drawdown_bps": round(mdd(pnls), 4),
        "top_session_trade_share": round(max_ts / n, 6) if n else 0.0,
...
def main(argv=None) -> int:
    ...
    rows = load(Path(a.input), a.instrument)
    sessions = defaultdict(list)
    for r in rows:
        sessions[sk(r)].append(r)
    ordered = sorted(sessions.items())
    sm = prior_state(ordered)
    hs = generate(a.instrument)
    results = [evaluate(h, ordered, sm, a.cost_bps, a.min_trades) for h in hs]
    results.sort(key=lambda x: (x["status"] == "PROMISING_NOT_CERTIFIED", x["net_expectancy_bps"], x["trades"]), reverse=True)

    run_id = datetime.now(timezone.utc).strftime("STATE-STRICT-%Y%m%dT%H%M%SZ")
    out = Path(a.output_dir) / run_id
    out.mkdir(parents=True, exist_ok=True)
    (out / "results.json").write_text(json.dumps(results, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_csv(out / "leaderboard.csv", results)
    manifest = {
        "schema_version": "tradebot-state-conditioned-screen-v2",
        "run_id": run_id,
        "instrument": a.instrument.upper(),
        "input": str(Path(a.input).resolve()),
        "input_sha256": hashlib.sha256(Path(a.input).read_bytes()).hexdigest(),
        "loaded_rows": len(rows),
        "sessions": len(sessions),
        "sessions_with_prior_state": len(sm),
        "sessions_without_prior_state": len(sessions) - len(sm),
        "hypotheses": len(hs),
        "promising_not_certified": sum(r["status"] == "PROMISING_NOT_CERTIFIED" for r in results),
        "min_trades": a.min_trades,
        "cost_bps": a.cost_bps,
        "families": sorted({h["family"] for h in hs}),
        "regimes": sorted({h["regime"] for h in hs}),
        "certification": "NOT_CERTIFIED",
        "runtime_authority": "NONE",
        "broker_actions_allowed": False,
    }
