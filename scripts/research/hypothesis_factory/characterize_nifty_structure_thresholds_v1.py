...
EXPECTED_NIFTY_SHA256='6a145d4d17f124f9dc8ee272c5a19ca98988873a14b294765f44a27284d8b7e8'
...
def main(argv=None):
    ap=argparse.ArgumentParser();ap.add_argument('--repo-root',default='.');ap.add_argument('--input',default='research/hypotheses/historical_corpus/kite_nifty_cache_v2/canonical/NIFTY.csv');ap.add_argument('--output',default='research/evidence/market_structure_pattern_atlas_v1/NIFTY_threshold_characterization_v1.json');a=ap.parse_args(argv)
    root=Path(a.repo_root).resolve();ip=Path(a.input);ip=ip if ip.is_absolute() else root/ip;op=root/a.output
    ...
    try:
        if not ip.exists():raise ValueError('canonical_nifty_ohlc_missing')
        actual_sha=sha256(ip)
        if actual_sha!=EXPECTED_NIFTY_SHA256:raise ValueError('canonical_nifty_hash_mismatch:'+actual_sha)
        ...
        res.update({'status':'NIFTY_STRUCTURE_THRESHOLD_CHARACTERIZATION_COMPLETE','instrument':'NIFTY','dataset_path':str(ip),'dataset_sha256':actual_sha,'expected_dataset_sha256':EXPECTED_NIFTY_SHA256,'rows':len(rows),'sessions':len({r['session'] for r in rows}),'thresholds_bps':list(THRESHOLDS),'by_threshold':by,'interpretation':'Descriptive NIFTY structural-scale characterization on verified canonical full OHLC only. No forward outcomes, profitability, strategy selection, or edge claim.'})
