...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--reconciled-manifest", default="")
    args = p.parse_args(argv)

    instrument = args.instrument.strip().upper()
    cache_dir = Path(args.cache_dir)
    if args.reconciled_manifest:
        manifest_path = Path(args.reconciled_manifest)
        dataset_kind = "RECONCILED_CANONICAL"
    else:
        manifest_path = cache_dir / "cache_manifest.json"
        dataset_kind = "CANONICAL_CACHE"
    if not manifest_path.exists():
        raise SystemExit(f"source manifest missing: {manifest_path}")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if args.reconciled_manifest:
        canonical = manifest.get("outputs", {}).get(instrument)
    else:
        canonical = manifest.get("canonical_outputs", {}).get(instrument)
    if not canonical:
        raise SystemExit(f"no cached dataset for instrument={instrument}")
    ...
    hf.write_json(out / "run_manifest.json", manifest_out)
