...
EXPECTED_DATASET_SHA='66ddbfead966388262b9a1e49937fb227b711ce32f70eaf715a93a5e32572b32';EXPECTED_GENERATION='HUNT_V7_GENERATION_FREEZE'
...
def direction(pid,rows,i,c):
 r=rows[i];br=f(r,'banknifty_ret_1_bps');bfo=f(r,'banknifty_from_open_bps');nfo=f(r,'nifty_from_open_bps');sfo=f(r,'sensex_from_open_bps');bi=bar_index(rows,i)
 if not all(math.isfinite(x) for x in (br,bfo,nfo,sfo)):return 0
 if pid in ('HUNT_V7_MIDSESSION_FROM_OPEN_CONTINUATION','HUNT_V7_MIDSESSION_FROM_OPEN_FADE'):
  if bi<c['min_bar_index'] or bi>c['max_bar_index'] or abs(bfo)<c['open_bps'] or sgn(bfo)==0:return 0
  if pid.endswith('CONTINUATION'):return sgn(bfo) if sgn(br)==sgn(bfo) else 0
  return sgn(br) if sgn(br)==-sgn(bfo) and sgn(br)!=0 else 0
 if pid=='HUNT_V7_LATE_SESSION_LEADER_CONSENSUS':
  if bi<c['bar_index_min'] or sgn(nfo)==0 or sgn(nfo)!=sgn(sfo) or min(abs(nfo),abs(sfo))<c['leader_open_bps']:return 0
  return sgn(nfo) if sgn(br)==sgn(nfo) else 0
 if pid=='HUNT_V7_LATE_SESSION_CROSS_MARKET_DISLOCATION_FADE':
  if bi<c['bar_index_min'] or sgn(nfo)==0 or sgn(nfo)!=sgn(sfo) or sgn(bfo)!=-sgn(nfo) or abs(bfo)<c['bn_open_bps']:return 0
  return sgn(nfo)
...
def main(argv=None):
 ap=argparse.ArgumentParser();ap.add_argument('--repo-root',default='.');ap.add_argument('--dataset',default='research/hypotheses/cross_market/BANKNIFTY_NIFTY_SENSEX.csv');ap.add_argument('--freeze',default='research/strategy_certification/passports/HUNT_V7_GENERATION_FREEZE.json');ap.add_argument('--output',default='research/evidence/strategy_certification/HUNT_V7_DEVELOPMENT_SCREEN.json');a=ap.parse_args(argv);root=Path(a.repo_root).resolve();ds=root/a.dataset;fp=root/a.freeze;out=root/a.output
