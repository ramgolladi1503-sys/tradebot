...
def run_benchmark():
    ...
    cycles = 1000
    html_payload = (
        "<html><body>Subject: Benchmark Circular 123. Date: May 01, 2026. "
        + "A" * 5000
        + "</body></html>"
    )
    ...
    print(
        f"- **Parser Throughput (HTML Normalization + Regex)**: `{parse_ops:.2f} ops/sec`"
    )
    ...
    print(
        f"- **SQLite Throughput (Inserts with WAL enabled)**: `{sqlite_ops:.2f} ops/sec`"
    )
    ...
    print(
        f"- **Telemetry Throughput (JSONL Serialization + IO)**: `{tel_ops:.2f} ops/sec`"
    )
    ...
    print(
        f"- **Report Generation (Full Offline Markdown Dump)**: `{rep_ops:.2f} ops/sec`"
    )
