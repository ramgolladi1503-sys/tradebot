...
def run_offline_aeron7_research(
    *,
    ...
    convert_report = convert_aeron7_intraday(source_root=src_root, output_dir=raw_dir, symbols=symbols)
    ...
    if label_reports:
        ...
        segmented_report = train_segmented_offline_models(
            ...
            model_dir=model_dir,
            label_column="label_up" if len(horizons_bars) <= 1 else f"label_up_{horizons_bars[0]}",
    ...
    if ev_reports:
        regime_report = build_regime_model_comparison(
            ...
            label_column="ev_positive" if len(horizons_bars) <= 1 else f"ev_positive_{horizons_bars[0]}",
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the offline Aeron7 research pipeline end to end.")
    ...
    parser.add_argument("--work-dir", default="", help="Workspace for generated artifacts")
    parser.add_argument("--symbols", default="", help="Comma-separated Aeron7 symbols")
    parser.add_argument("--horizons", default="", help="Comma-separated label horizons in bars")
    ...
    report = run_offline_aeron7_research(
        ...
        train_window_days=args.train_window_days or int(config.get("train_window_days", 60)),
    ...
    if report.get("walk_forward_report"):
        print(f"walk_forward_windows={report['walk_forward_report']['config']['window_count']}")
    return 0
