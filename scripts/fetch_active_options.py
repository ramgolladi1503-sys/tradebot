...
def main():
    ...
    for inst in nse_insts:
        if inst["tradingsymbol"] == "NIFTY 50": nifty_token = inst["instrument_token"]
        if inst["tradingsymbol"] == "NIFTY BANK": banknifty_token = inst["instrument_token"]
    ...
    nifty_opts = [i for i in nfo_insts if i["name"] == "NIFTY" and i["instrument_type"] in ["CE", "PE"]]
    banknifty_opts = [i for i in nfo_insts if i["name"] == "BANKNIFTY" and i["instrument_type"] in ["CE", "PE"]]
    ...
    nifty_expiries = sorted(list(set([i["expiry"] for i in nifty_opts if i["expiry"] >= today])))
    banknifty_expiries = sorted(list(set([i["expiry"] for i in banknifty_opts if i["expiry"] >= today])))
    
    nifty_closest_exp = nifty_expiries[0]
    banknifty_closest_exp = banknifty_expiries[0]
    
    print(f"Closest Expiries: NIFTY={nifty_closest_exp}, BANKNIFTY={banknifty_closest_exp}")
    
    # Select ATM +/- 1 strike
    target_tokens = {
        "NIFTY_INDEX": nifty_token,
        "BANKNIFTY_INDEX": banknifty_token
    }
    ...
    for opt in banknifty_opts:
        if opt["expiry"] == banknifty_closest_exp:
            if opt["strike"] in [banknifty_atm - 100, banknifty_atm, banknifty_atm + 100]:
                target_tokens[opt["tradingsymbol"]] = opt["instrument_token"]
    ...
    print(f"Successfully merged {len(sorted_ticks)} synchronous ticks across multiple assets.")
    
    out_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'active_options_replay.json')
