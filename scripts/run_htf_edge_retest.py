...
def main():
    ...
    for s_name in STRATEGIES:
        ...
        for tr in trades:
            ...
            future = (
                day_1m[day_1m["timestamp"] > entry_t]
            ...
            pts = (
                (exit_price - entry_p) if direction == "CE" else (entry_p - exit_price)
            )
            # scale by delta 0.5
            opts = pts * 0.5
        ...
        status = (
            ...
            if rx > 0
            else "COST_KILLED_AFTER_CORRECT_IMPLEMENTATION"
        )
    ...
    with open("docs/strategy_research/htf_final_classification.md", "w") as f:
        ...
        for r in results:
            f.write(
                f"| {r['Strategy']} | {r['Trades']} | {r['Win_Rate']:.2%} | {r['Gross_Exp']:.2f} | {r['Realistic_Net_Exp']:.2f} | {r['Status']} |\n"
            )
