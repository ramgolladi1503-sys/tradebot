...
def build_blockers_report(rows: Sequence[Mapping[str, Any]], day: date | str) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        entry_status = str(
            ...
            or decision_trace.get("entry_status")
            or "UNKNOWN"
        )
...
def write_blockers_report(report: Mapping[str, Any], out_dir: Path) -> tuple[Path, Path]:
    ...
    _atomic_write(json_path, json.dumps(dict(report), ensure_ascii=True, indent=2, sort_keys=True) + "\n")
    _atomic_write(md_path, render_blockers_md(report))
    return md_path, json_path


def run_blockers_report(day: date | str, *, source_paths: Iterable[Path] | None = None, out_base: Path | None = None) -> tuple[dict[str, Any], Path, Path]:
    target_day = _coerce_day(day)
    ...
    out_root = Path(out_base) if out_base is not None else Path("runtime/analytics/reports")
    md_path, json_path = write_blockers_report(report, out_root)
    return report, md_path, json_path


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate blockers rollup from suggestion logs")
    parser.add_argument("--date", dest="day", default=None, help="Target date (YYYY-MM-DD). Defaults to yesterday (IST).")
    parser.add_argument(
        "--source",
        dest="source_paths",
        action="append",
    ...
    report, md_path, json_path = run_blockers_report(args.day, source_paths=paths, out_base=Path(args.out_base))
    print(json.dumps({"day": report.get("day"), "md": str(md_path), "json": str(json_path)}, ensure_ascii=True))
    return 0
