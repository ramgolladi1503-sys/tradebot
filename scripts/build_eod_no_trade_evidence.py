...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build read-only EOD no-trade evidence from tick/replay/runtime artifacts."
    )
    parser.add_argument(
        "--date", required=True, help="Trade date in YYYY-MM-DD format."
    )
    parser.add_argument("--tick-file", required=True, help="Raw tick JSONL file.")
    parser.add_argument(
        "--replay-file",
        default="data/active_options_replay.json",
        help="Converted replay JSON file.",
    )
    parser.add_argument(
        "--runtime-dir", default=".runtime", help="Runtime artifact directory."
    )
    parser.add_argument(
        "--wfa-csv", default="data/oos_trades.csv", help="Proxy WFA CSV path."
    )
    parser.add_argument("--json-out", required=True, help="Output JSON evidence path.")
    parser.add_argument(
        "--markdown-out", required=True, help="Output Markdown evidence path."
    )
