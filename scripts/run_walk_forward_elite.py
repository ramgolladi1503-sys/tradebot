...
def run_walk_forward(data: pd.DataFrame):
    ...
    param_grid = list(
        product(
            horizons, slippage_bps, target_atrs, stop_atrs, ts_act_mults, ts_trail_mults
        )
    )
    ...
    for fold in range(4):
        ...
        tasks_is = [
            (train_data, hz, sl, tgt, stp, ts_act, ts_trail)
            for hz, sl, tgt, stp, ts_act, ts_trail in param_grid
        ]

        print(
            f"Starting Elite IS Grid Search for Fold {fold + 1} ({len(train_data)} rows)..."
        )
    ...
    if pf_oos > 1.2 and expectancy > 0 and stability_penalty <= 2:
        print(
            f"PROMOTED! Strategy meets elite robustness criteria (OOS PF: {pf_oos:.2f} > 1.2, Expectancy: {expectancy:.2f} > 0, Stability Penalty: {stability_penalty} <= 2)."
        )
    else:
        print(
            f"REJECTED. Failed robustness check (OOS PF: {pf_oos:.2f}, Expectancy: {expectancy:.2f}, Stability Penalty: {stability_penalty})."
        )
...
def run_option_backtest(csv_path: str):
    ...
    if metrics.get("after_cost_expectancy", 0) > 0:
        ...
        print(
            f"Win Rate:           {metrics.get('win_rate', 0) * 100:.2f}% (IRRELEVANT)"
        )
