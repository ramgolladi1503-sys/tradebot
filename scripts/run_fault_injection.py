...
from core.intelligence.extractors.hardened_base import (
    HardenedBaseExtractor,
    ExtractionError,
)
...
class ChaosFetcher(BaseFetcher):
    ...
    def _execute_fetch(self, url: str) -> Dict[str, Any]:
        if self.mode == "timeout":
            ...
            return {
                "raw_content": "<html><head><title>P",
                "size_bytes": 22,
                "content_type": "text/html",
            }
        return {
            "raw_content": "<html><body>Subject: OK. Date: May 01, 2026</body></html>",
            "size_bytes": 100,
            "content_type": "text/html",
        }
...
def run_fault_injection():
    ...
    def test_fault(
        name: str, fetch_mode: str, extract_html: str, db_locked: bool = False
    ):
        try:
            ...
            if status == "success" and payload:
                ...
                telemetry.emit_extraction_event(
                    ...
                    else "extraction_succeeded",
                    "CHAOS",
                    extracted["status"],
                    extracted.get("document_hash"),
                )
            else:
            ...
            print(
                f"| `{name}` | `{'Fetch' if fetch_mode != 'ok' else 'Extract/DB'}` | YES | NO | YES | NONE |"
            )
        except Exception as e:
            if "database is locked" in str(e):
                print(
                    f"| `{name}` | `Storage` | YES (Caught generic) | NO | YES | NONE |"
                )
            else:
