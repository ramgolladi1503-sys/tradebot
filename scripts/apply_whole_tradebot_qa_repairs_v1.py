...
def replace_once(path: str, old: str, new: str) -> None:
    ...
    if count != 1:
        raise RuntimeError(f"expected exactly one match path={path} count={count} old={old[:120]!r}")
...
def patch_opportunity_truth() -> None:
    fallback_mutation = '''    if source_flags.get("quote_source") == "REST_RECOVERY" or source_flags.get("recovered_fallback"):
...
'''
    pure_fallback = '''    if source_flags.get("quote_source") == "REST_RECOVERY" or source_flags.get("recovered_fallback"):
        # Classification must be pure. Trade is a frozen dataclass and truth
        # evaluation must never mutate the candidate it is inspecting.
        execution_allowed = False
'''
    replace_once("core/opportunity_engine.py", fallback_mutation, pure_fallback)
    replace_once("core/opportunity_engine.py", fallback_mutation, pure_fallback)
...
'''
    pure_ml = '''                # Preserve the frozen candidate and report the blocker through
                # score metrics. Annotation later creates a replaced copy.
                metrics["ml_acceptance_blocker"] = "ml_probability_too_low"
                metrics["ml_tradable_reasons_blocking"] = list(new_blockers)
                return "NEAR_EXECUTABLE"
'''
    replace_once("core/opportunity_engine.py", ml_mutation, pure_ml)
...
def patch_archive_security() -> None:
    replace_once(
...
def _safe_extract_tar(archive: tarfile.TarFile, root: Path) -> None:
    """Extract regular files/directories without traversal, links or devices."""
...
def _evidence_root(source: str | Path) -> Iterator[Path]:
'''
    replace_once("core/evidence_replay_report.py", marker, helper)
    replace_once(
        "core/evidence_replay_report.py",
...
def patch_xml_and_permissions() -> None:
    replace_once(
    ...
    if "defusedxml==0.7.1" not in lines:
        insert_at = lines.index("requests") + 1 if "requests" in lines else len(lines)
        lines.insert(insert_at, "defusedxml==0.7.1")
        requirements.write_text("\n".join(lines) + "\n", encoding="utf-8")


def patch_url_transports() -> None:
    write_file(
...
class HTTPFetcher(BaseFetcher):
    """Standard-library HTTP fetcher with explicit URL and size boundaries."""
    ...
    def _execute_fetch(self, url: str) -> Dict[str, Any]:
        ...
        with urllib.request.urlopen(  # nosec B310 - scheme/host/IP validated above
            req,
...
def _default_transport(request: urllib.request.Request, timeout: float) -> bytes:
    ...
    with urllib.request.urlopen(  # nosec B310 - exact HTTPS host and port validated above
        request,
        timeout=timeout,
    ) as response:
        return response.read()
...
def patch_secure_temp_path() -> None:
    replace_once(
...
def patch_secure_kite_hash() -> None:
    replace_once(
        "scripts/build_secure_kiteconnect_wheel.py",
        'UPSTREAM_WHEEL_SHA256 = "8670ca79428cc46fbce2b02dc59de4ba2c4ca68b40820b179836389cfa538e19"',
        'UPSTREAM_WHEEL_SHA256 = "4601a48355aeeae4ac8857d44f292aef3847351840e0c18aa8f5d3c5afcf92a7"',
    )


def add_security_tests() -> None:
    write_file(
...
def test_evidence_archive_extracts_regular_files_with_private_modes(tmp_path):
...
def test_http_fetcher_accepts_public_http_and_https_urls():
...
def main() -> None:
    patch_opportunity_truth()
    patch_archive_security()
    patch_xml_and_permissions()
    patch_url_transports()
    patch_secure_temp_path()
    patch_non_security_hashes()
    patch_secure_kite_hash()
    add_security_tests()
