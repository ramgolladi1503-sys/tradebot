...
def _read_intents(path: Path) -> list[OptionIntent]:
    ...
    for row in frame.to_dict("records"):
        ...
        earliest = pd.Timestamp(earliest_raw) if earliest_raw and not pd.isna(earliest_raw) else signal + pd.Timedelta(minutes=1)
        out.append(
            OptionIntent(
    ...
    return sorted(out, key=lambda item: (item.signal_timestamp, item.strategy_id, item.signal_identity_hash))
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    option_root = Path(args.option_root).resolve()
    ...
    _write_csv(output_root / "authority_backed_contract_inventory.csv", inventory_rows)
    grouped: dict[tuple[str, str], list[Any]] = defaultdict(list)
    for trade in trades:
        grouped[(trade.strategy_id, trade.partition)].append(trade)
    metric_rows = []
    for (strategy_id, partition), rows in sorted(grouped.items()):
        metric_rows.append({"strategy_id": strategy_id, "partition": partition, **metrics(rows)})
    ...
    manifest = {
        ...
        "option_root": str(option_root),
        "intent_count": len(intents),
        "authority_backed_contract_count": len(inventory),
        "trade_count": len(trades),
        "blocker_count": len(blockers),
        "partition_dates": {name: sorted(day.isoformat() for day in dates) for name, dates in partitions.items()},
        ...
        "verdict": "INSUFFICIENT_REAL_OPTION_HISTORY" if len({item.expiry for item in inventory}) < 3 else "OPTION_REPLAY_COMPLETED",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Replay canonical TradeBot signal intents against real expired option minute candles.")
    parser.add_argument("--option-root", required=True, help="Root containing raw/responses and normalized folders")
    parser.add_argument("--intents-csv", required=True, help="Canonical option-intent CSV from the directional campaign")
    ...
    parser.add_argument("--authorize-holdout", action="store_true", help="Explicitly authorize holdout outcome reads")
