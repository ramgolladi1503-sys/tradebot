...
def _add_ev_columns(frame: pd.DataFrame, *, cost_bps: float) -> pd.DataFrame:
    ...
    for col in future_cols:
        ...
        out[ev_col] = pd.to_numeric(out[col], errors="coerce").fillna(0.0) - (
            cost_bps / 10000.0
        )
    ...
    *,
    ...
    if horizons_bars:
        report = build_multi_horizon_labels(
            input_csv=source, output_csv=output_csv, horizons_bars=horizons_bars
        )
    else:
        report = build_next_bar_labels(
            input_csv=source, output_csv=output_csv, horizon_bars=horizon_bars
        )
    ...
    ev_cols = [
        ...
        if col.startswith("expected_value") or col.startswith("ev_positive")
    ]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build offline labels with an explicit trade-level EV target."
    )
    parser.add_argument("--input-csv", required=True)
    parser.add_argument("--output-csv", required=True)
    parser.add_argument("--horizon-bars", type=int, default=1)
    parser.add_argument(
        "--multi-horizons", default="", help="Optional comma-separated horizons"
    )
    parser.add_argument(
        "--cost-bps", type=float, default=10.0, help="Estimated per-trade cost in bps"
    )
