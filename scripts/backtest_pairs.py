...
class MockExecutionRouter:
    ...
    def execute(self, trade, bid, ask, volume, **kwargs):
        ...
        if (
            ...
            and "unwind" in trade.get("strategy_id", "").lower()
        ):
...
def generate_synthetic_data(num_points=500):
    ...
    for i in range(num_points):
        ...
        data.append({"NIFTY_INDEX": nifty, "BANKNIFTY_INDEX": banknifty})
    return data
...
def main():
    ...
    parser.add_argument(
        "--use-real-data", action="store_true", help="Use real historical data"
    )
    ...
    if args.use_real_data:
        ...
        data_path = os.path.join(
            os.path.dirname(__file__), "..", "data", "pairs_historical.json"
        )
        if not os.path.exists(data_path):
            print(
                f"ERROR: {data_path} not found. Run scripts/fetch_historical_pairs.py first."
            )
            return
    ...
    core.cross_asset.cfg.CROSS_ASSET_SYMBOLS = {
    ...
    try:
        for i, prices in enumerate(data):
            ...
            if i == 900:
                ...
                logger.info(
                    f"CFG SYMBOLS: {getattr(core.cross_asset.cfg, 'CROSS_ASSET_SYMBOLS', {})}"
                )
                import json
                ...
                if "x_banknifty_nifty_spread_z" not in features:
                    logger.info(
                        f"Missing spread_z. Available keys: {list(features.keys())}"
                    )
            ...
            for intent in getattr(report, "generated_intents", []):
                logger.info(
                    f"Generated Intent at Step {i}: {intent.direction} Spread (Z: {intent.metadata.get('spread_z', 0):.2f})"
                )
                ...
                if res.get("status") == "FILLED":
                    ...
                    unwinds_taken += 1
                    logger.warning(
                        f"  -> UNWOUND: Leg B failed, Unwind sequence activated"
                    )
        ...
        print(
            f"Strategy Success: True Cointegration tracking and Unwind logic verified."
        )
    finally:
