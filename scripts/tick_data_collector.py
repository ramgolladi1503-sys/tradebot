...
def get_target_tokens() -> dict[int, str]:
    ...
    nifty_opts = [i for i in nfo_insts if i["name"] == "NIFTY" and i["instrument_type"] in ["CE", "PE"] and i["expiry"] >= today]
    banknifty_opts = [i for i in nfo_insts if i["name"] == "BANKNIFTY" and i["instrument_type"] in ["CE", "PE"] and i["expiry"] >= today]
    ...
    target_tokens = {}
    if nifty_token: target_tokens[nifty_token] = "NIFTY 50"
    if banknifty_token: target_tokens[banknifty_token] = "NIFTY BANK"
        ...
        for t in ticks:
            try:
                record = {
                    ...
                    "symbol": target_tokens.get(t.get("instrument_token"), str(t.get("instrument_token"))),
                    "ltp": t.get("last_price"),
                    "bid": t.get("depth", {}).get("buy", [{}])[0].get("price") if "depth" in t and t["depth"].get("buy") else None,
                    "ask": t.get("depth", {}).get("sell", [{}])[0].get("price") if "depth" in t and t["depth"].get("sell") else None,
