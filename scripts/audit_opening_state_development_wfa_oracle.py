...
def main():
    ...
    # Calculate overall metrics independently
    def get_return(entry, exit, direction):
        if direction == "LONG": return (exit - entry)/entry
        if direction == "SHORT": return (entry - exit)/entry
        return 0.0
        
    for friction_bps, friction_val in [("0bps", 0.0), ("2bps", 0.0002), ("5bps", 0.0005), ("10bps", 0.0010)]:
        rets = [get_return(o["entry_price"], o["exit_price"], o["direction"]) - 2*friction_val for o in outcomes]
        n = len(rets)
        ...
        if isinstance(pf_val, dict): pf_val = None
        check(pf_val, pf, f"OVERALL ALL {friction_bps} profit_factor")
