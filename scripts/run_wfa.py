...
def main():
    ...
    parser.add_argument("--data", type=str, help="Path to CSV containing historical data")
    parser.add_argument("--slippage-bps", type=float, default=20.0, help="Slippage in bps (20.0 = 0.2%)")
    ...
    if args.data and os.path.exists(args.data):
        ...
        print("No valid data path provided. Generating 5 years of synthetic OHLCV data...")
        df = generate_dummy_data(years=5)
        
    print(f"Data shape: {df.shape}, Dates: {df.index.min().date()} to {df.index.max().date()}")
    ...
    if oos_trades.empty:
        ...
        print(oos_trades[['entry_price', 'exit_price', 'pl', 'cumulative_pl', 'wfa_window']].head(10))
        print("...")
        print(oos_trades[['entry_price', 'exit_price', 'pl', 'cumulative_pl', 'wfa_window']].tail(10))
