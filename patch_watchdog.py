...
new_func = """def _run_db_tick_watchdog_cycle(
    *,
    ...
    ws_transport_ok = last_any_packet_age <= 10.0
    ...
    stale_anchor_tokens = [
        ...
        if token in _LAST_MSG_TS_BY_TOKEN and _LAST_MSG_TS_BY_TOKEN[token] is not None and (now_epoch - _LAST_MSG_TS_BY_TOKEN[token]) > 15.0
    ]

    anchor_fresh_ok = len(stale_anchor_tokens) == 0

    stale_active_trade_tokens = [
        token for token in active_trade_tokens
        if token in _LAST_MSG_TS_BY_TOKEN and _LAST_MSG_TS_BY_TOKEN[token] is not None and (now_epoch - _LAST_MSG_TS_BY_TOKEN[token]) > 10.0
    ]

    # Write the stale trade tokens to global state so health_gate can read them
    global _STALE_ACTIVE_TRADE_TOKENS
    _STALE_ACTIVE_TRADE_TOKENS = set(stale_active_trade_tokens)
    ...
    if not market_open:
        ...
        _emit_feed_health(
            "FEED_HEALTH_OK",
            {
                ...
                "stale_active_trade_tokens": stale_active_trade_tokens,
                "stale_strikes": 0,
            },
        )
    else:
        stale_source = "transport" if not ws_transport_ok else "anchors"
        stale_age = last_any_packet_age if not ws_transport_ok else 15.1
