...
_PROFILES = {
    ...
    "RANGE": {"setup_family": "MEAN_REVERSION", "vwap_buffer_mult": 1.15, "min_move_mult": 0.8, "score_bias": -0.01, "range_extension_mult": 1.2},
}
...
def generate_signal(ltp, vwap, bias, vwap_buffer=0.0015, min_move=0.001, debug_stats=None, regime=None, expiry_context=False):
    """Canonical SENSEX VWAP signal; rejected evidence never becomes a signal."""
    ...
    abs_diff = abs(diff)
    if abs_diff < float(min_move) * 0.6 or diff == 0:
