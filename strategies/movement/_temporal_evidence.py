...
Every returned bar must have a parseable completion timestamp, be strictly
ordered, and be strictly earlier than StrategyContext.ts_epoch when a decision
time is supplied.
"""
...
def _bar_epoch(bar: dict[str, Any]) -> float | None:
    # Prefer completion time. A bar is not causal evidence until it has ended.
    for key in ("bar_end_timestamp","ts_epoch","timestamp","datetime","time","date","bar_start_timestamp"):
        if key in bar:
...
def compression_window(ctx: StrategyContext, *, lookback: int=6) -> tuple[float,float,float] | None:
    ...
    highs=[bar_value(b,"high") for b in window]; lows=[bar_value(b,"low") for b in window]; close=bar_value(window[-1],"close")
    if close is None or close<=0 or any(v is None for v in highs+lows):
        return None
    upper=max(float(v) for v in highs if v is not None); lower=min(float(v) for v in lows if v is not None)
    return upper,lower,(upper-lower)/abs(close)


def failed_break_reentry(ctx: StrategyContext, *, level: float, side: str, min_break_distance_pct: float) -> dict[str,float] | None:
    bars=completed_bars_before(ctx,min_bars=2)
    if not bars or level<=0:
        return None
    break_index=None; break_extreme=None
    for i,bar in enumerate(bars[:-1]):
        if side=="UP":
            extreme=bar_value(bar,"high")
            if extreme is not None and (extreme-level)/abs(level)>=min_break_distance_pct:
                break_index,break_extreme=i,extreme; break
        else:
