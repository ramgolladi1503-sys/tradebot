...
def generate_failed_breakout_trap_candidates(
    ...
    if bull_trap_score >= MIN_TRAP_EVIDENCE_SCORE:
        candidates.append(_build_candidate(ctx, regime, "BUY_PUT", bull_trap_score, "failed_upside_breakout_reentry"))

    bear_trap_score = _bear_trap_score(ctx, regime)
    if bear_trap_score >= MIN_TRAP_EVIDENCE_SCORE:
        candidates.append(_build_candidate(ctx, regime, "BUY_CALL", bear_trap_score, "failed_downside_breakdown_reentry"))
...
def _bull_trap_score(ctx: StrategyContext, regime: MovementRegimeResult) -> float:
    ...
    explicit_reentry = _metadata_bool(ctx, "price_reentered_range") or _metadata_bool(ctx, "failed_breakout_confirmed")
    if not had_break and not explicit_reentry:
        return 0.0
    option_stall = _option_stall_for_failed_upside(ctx)
    return clamp_score(
        0.40 * ratio_score(reentry_distance, start=MIN_FAILED_BREAK_DISTANCE_PCT, full=MAX_REENTRY_DISTANCE_PCT)
        + 0.25 * clamp_score(regime.scores.get("TRAP_RISK", 0.0))
        + 0.20 * option_stall
...
def _bear_trap_score(ctx: StrategyContext, regime: MovementRegimeResult) -> float:
    ...
    explicit_reentry = _metadata_bool(ctx, "price_reentered_range") or _metadata_bool(ctx, "failed_breakdown_confirmed")
    if not had_break and not explicit_reentry:
        return 0.0
